...
var ProcessWorker = class {
  ...
  postMessage(message, transferListItem) {
    ...
    if (this.port)
      this.port.on("message", (message$1) =>
        this.send({
          ...message$1,
          source: "port",
  ...
  *[Symbol.iterator]() {
  ...
  maybeAvailable(item) {
    ...
    if (item.currentUsage() < this.maximumUsage)
      for (const listener of this.onAvailableListeners) listener(item);
  }
  onAvailable(fn) {
...
var WorkerInfo = class extends AsynchronouslyCreatedResource {
  ...
  async destroy(timeout) {
      ...
      await new Promise((resolve$1, reject$1) => {
        this.postTask(
          new TaskInfo(
            ...
            filename,
            teardown,
            (error, result) => (error ? reject$1(error) : resolve$1(result)),
            null,
            1,
            void 0,
          ),
    ...
    const timer = timeout
      ? setTimeout(() => reject(new Error("Failed to terminate worker")), timeout)
      : null;
...
var ThreadPool = class {
  ...
  _addNewWorker() {
    ...
    const worker =
      this.options.runtime === "child_process" ? new ProcessWorker() : new ThreadWorker();
    worker.initialize({
  ...
  runTask(task, options) {
    ...
    if (this.taskQueue.size > 0) {
      ...
      if (this.taskQueue.size >= totalCapacity)
        if (this.options.maxQueue === 0) return Promise.reject(Errors.NoTaskQueueAvailable());
  ...
  shouldRecycleWorker(taskInfo) {
    ...
    if (
      ...
      this.options.maxMemoryLimitBeforeRecycle !== void 0 &&
...
var Tinypool = class extends EventEmitterAsyncResource {
  ...
  constructor(options = {}) {
    ...
    if (
      options.minThreads !== void 0 &&
      options.maxThreads !== void 0 &&
  ...
  static move(val) {
    if (val != null && typeof val === "object" && typeof val !== "function") {
      if (!isTransferable(val))
        if (types.isArrayBufferView(val)) val = new ArrayBufferViewTransferable(val);
