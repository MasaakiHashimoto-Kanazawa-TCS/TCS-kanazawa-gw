import { r as __toESM } from "./chunks/chunk.js";
import {
  F as VITE_PLUS_VERSION,
  n as createLogger,
  z as require_picocolors,
...
if (!checkNodeVersion(process.versions.node))
  console.warn(
    import_picocolors.default.yellow(
      `You are using Node.js ${process.versions.node}. Vite requires Node.js version 20.19+ or 22.12+. Please upgrade your Node.js version.`,
    ),
  );
const cli = cac("vp");
    ...
    try {
      ...
      const modeString =
        options.mode && options.mode !== "development"
          ? `  ${import_picocolors.default.bgGreen(` ${import_picocolors.default.bold(options.mode)} `)}`
          : "";
      const viteStartTime = global.__vite_start_time ?? false;
      const startupDurationString = viteStartTime
        ? import_picocolors.default.dim(
            `ready in ${import_picocolors.default.reset(import_picocolors.default.bold(Math.ceil(performance.now() - viteStartTime)))} ms`,
          )
        : "";
      ...
      logger.error(
        import_picocolors.default.red(`error when starting dev server:\n${inspect(e)}`),
        { error: e },
      );
...
cli
  ...
  .option("--target <target>", `[string] transpile target (default: 'baseline-widely-available')`)
...
cli
  ...
  .action(async (root, options) => {
    ...
    try {
      ...
      createLogger(options.logLevel).error(
        import_picocolors.default.red(`error when optimizing deps:\n${inspect(e)}`),
        { error: e },
      );
      process.exit(1);
    }
...
cli
  ...
  .action(async (root, options) => {
    ...
    try {
      ...
      createLogger(options.logLevel).error(
        import_picocolors.default.red(`error when starting preview server:\n${inspect(e)}`),
        { error: e },
      );
      process.exit(1);
    } finally {
