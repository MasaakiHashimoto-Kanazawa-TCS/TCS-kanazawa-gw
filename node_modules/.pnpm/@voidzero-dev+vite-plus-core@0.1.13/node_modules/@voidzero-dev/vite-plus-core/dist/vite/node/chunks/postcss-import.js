...
var require_base64_encoded_import = /* @__PURE__ */ __commonJSMin((exports, module) => {
  ...
  module.exports = function base64EncodedConditionalImport(prelude, conditions) {
    ...
    for (const condition of conditions)
      params = `'data:text/css;base64,${Buffer.from(`@import ${params}`).toString("base64")}' ${formatImportPrelude(condition.layer, condition.media, condition.supports)}`;
    return params;
  };
});
//#endregion
      ...
      if (
        ...
        stmt.node = stmt.node.clone({
          ...
          params: base64EncodedConditionalImport(
            `'data:text/css;base64,${Buffer.from(stmt.node.toString()).toString("base64")}'`,
            stmt.conditions,
          ),
        });
        return;
...
var require_parse_statements = /* @__PURE__ */ __commonJSMin((exports, module) => {
  ...
  function parseCharset(result, atRule, conditions, from) {
    if (atRule.prev())
      return result.warn("@charset must precede all other statements", { node: atRule });
    return {
      type: "charset",
      node: atRule,
      conditions: [...conditions],
  ...
  function parseImport(result, atRule, conditions, from) {
    ...
    for (let i = 0; i < params.length; i++) {
      ...
      if (node.type === "string") {
        if (stmt.uri)
          return result.warn(`Multiple url's in '${atRule.toString()}'`, { node: atRule });
        if (!node.value)
      ...
      if ((node.type === "word" || node.type === "function") && /^layer$/i.test(node.value)) {
        if (typeof layer !== "undefined")
          return result.warn(`Multiple layers in '${atRule.toString()}'`, { node: atRule });
...
var require_parse_styles = /* @__PURE__ */ __commonJSMin((exports, module) => {
  ...
  async function loadImportContent(result, stmt, filename, options, state, postcss) {
    ...
    const stmtDuplicateCheckKey = conditions
      .map((condition) => formatImportPrelude(condition.layer, condition.media, condition.supports))
      .join(":");
