...
const resolve = function (...arguments_) {
  ...
  for (let index = arguments_.length - 1; index >= -1 && !resolvedAbsolute; index--) {
    ...
    !path ||
      path.length === 0 ||
      ((resolvedPath = `${path}/${resolvedPath}`), (resolvedAbsolute = isAbsolute(path)));
  }
  return (
    (resolvedPath = normalizeString(resolvedPath, !resolvedAbsolute)),
    resolvedAbsolute && !isAbsolute(resolvedPath)
      ? `/${resolvedPath}`
...
const isAbsolute = function (p) {
  ...
  dirname = function (p) {
    ...
    return (
      segments.length === 1 && _DRIVE_LETTER_RE.test(segments[0]) && (segments[0] += "/"),
      segments.join("/") || (isAbsolute(p) ? "/" : ".")
    );
  },
...
function encodePathChars(filepath) {
  return (
    filepath.indexOf("%") !== -1 && (filepath = filepath.replace(percentRegEx, "%25")),
    !isWindows &&
      filepath.indexOf("\\") !== -1 &&
      (filepath = filepath.replace(backslashRegEx, "%5C")),
    filepath.indexOf("\n") !== -1 && (filepath = filepath.replace(newlineRegEx, "%0A")),
    filepath.indexOf("\r") !== -1 && (filepath = filepath.replace(carriageReturnRegEx, "%0D")),
    filepath.indexOf("	") !== -1 && (filepath = filepath.replace(tabRegEx, "%09")),
    filepath
  );
}
...
function posixPathToFileHref(posixPath) {
  ...
  return (
    (filePathLast === 47 || (isWindows && filePathLast === 92)) &&
      resolved[resolved.length - 1] !== "/" &&
      (resolved += "/"),
    (resolved = encodePathChars(resolved)),
    resolved.indexOf("?") !== -1 && (resolved = resolved.replace(questionRegex, "%3F")),
    resolved.indexOf("#") !== -1 && (resolved = resolved.replace(hashRegex, "%23")),
    new URL(`file://${resolved}`).href
  );
}
...
function decodeInteger(reader, relative) {
  let value = 0,
    shift = 0,
    integer = 0;
  do ((integer = charToInt[reader.next()]), (value |= (integer & 31) << shift), (shift += 5));
  while (integer & 32);
  ...
  do {
    ...
    for (genColumn = 0; reader.pos < semi; ) {
      ...
      ((genColumn = decodeInteger(reader, genColumn)),
        genColumn < lastCol && (sorted = !1),
        (lastCol = genColumn),
        hasMoreVlq(reader, semi)
...
function memoizedBinarySearch(haystack, needle, state, key) {
  let { lastKey, lastNeedle, lastIndex } = state,
    low = 0,
...
function originalPositionFor(map, needle) {
  ...
  let segments = decoded[line],
    index = traceSegmentInternal(segments, cast(map)._decodedMemo, line, column, bias || 1);
...
function traceSegmentInternal(segments, memo, line, column, bias) {
  ...
  return (
    found
      ...
      : bias === -1 && index++,
    index === -1 || index === segments.length ? -1 : index
  );
}
//#endregion
...
const MODULE_RUNNER_SOURCEMAPPING_REGEXP = RegExp(
...
var EvaluatedModuleNode = class {
  ...
  EvaluatedModules = class {
    ...
    getModuleSourceMapById(id) {
      ...
      let pattern = `//# ${SOURCEMAPPING_URL}=data:application/json;base64,`,
        lastIndex = mod.meta.code.lastIndexOf(pattern);
      if (lastIndex === -1) return null;
      let mapString = MODULE_RUNNER_SOURCEMAPPING_REGEXP.exec(mod.meta.code.slice(lastIndex))?.[1];
      return mapString
        ? ((mod.map = new DecodedMap(JSON.parse(decodeBase64(mapString)), mod.file)), mod.map)
        : null;
    }
    clear() {
...
function normalizeModuleId(file) {
  return prefixedBuiltins.has(file)
    ? file
    : slash(file)
        .replace(/^\/@fs\//, isWindows ? "" : "/")
        .replace(/^node:/, "")
        .replace(/^\/+/, "/")
        .replace(/^file:\/+/, isWindows ? "" : "/");
}
//#endregion
...
var HMRContext = class {
  ...
  HMRClient = class {
    ...
    warnFailedUpdate(err, path) {
      ((!(err instanceof Error) || !err.message.includes("fetch")) && this.logger.error(err),
        this.logger.error(
    ...
    async fetchUpdate(update) {
      let { path, acceptedPath, firstInvalidatedBy } = update,
        mod = this.hotModulesMap.get(path);
      if (!mod) return;
      let fetchedModule,
        isSelfUpdate = path === acceptedPath,
        qualifiedCallbacks = mod.callbacks.filter(({ deps }) => deps.includes(acceptedPath));
      ...
      return () => {
        try {
          ...
          for (let { deps, fn } of qualifiedCallbacks)
...
const createInvokeableTransport = (transport) => {
    ...
    if (!transport.send || !transport.connect)
      throw Error("transport must implement send and connect when invoke is not implemented");
    ...
    return {
      ...
      ...(invokeableTransport.disconnect
        ? {
            async disconnect() {
              isConnected &&
                (connectingPromise && (await connectingPromise),
                (isConnected = !1),
...
function createIsBuiltin(builtins) {
  let plainBuiltinsSet = new Set(builtins.filter((builtin) => typeof builtin == "string")),
    regexBuiltins = builtins.filter((builtin) => typeof builtin != "string");
    ...
    if (!(!hmrClient || runner.isClosed()))
      switch (payload.type) {
        ...
        case "custom":
          ...
          break;
        case "full-reload": {
          let { triggeredBy } = payload,
            clearEntrypointUrls = triggeredBy
              ? getModulesEntrypoints(runner, getModulesByFile(runner, slash(triggeredBy)))
...
function findAllEntrypoints(runner, entrypoints = /* @__PURE__ */ new Set()) {
  for (let mod of runner.evaluatedModules.idToModuleMap.values())
...
function resetInterceptor(runner, options) {
  (evaluatedModulesCache.delete(runner.evaluatedModules),
    options.retrieveFile && retrieveFileHandlers.delete(options.retrieveFile),
    options.retrieveSourceMap && retrieveSourceMapHandlers.delete(options.retrieveSourceMap),
    evaluatedModulesCache.size === 0 &&
      ((Error.prepareStackTrace = originalPrepare), (overridden = !1)));
}
function interceptStackTrace(runner, options = {}) {
  return (
    (overridden ||= ((Error.prepareStackTrace = prepareStackTrace), !0)),
    evaluatedModulesCache.add(runner.evaluatedModules),
    options.retrieveFile && retrieveFileHandlers.add(options.retrieveFile),
    options.retrieveSourceMap && retrieveSourceMapHandlers.add(options.retrieveSourceMap),
    () => resetInterceptor(runner, options)
  );
}
function supportRelativeURL(file, url) {
  ...
  return protocol && /^\/\w:/.test(startPath)
    ? ((protocol += "/"), protocol + slash(posixResolve(startPath, url)))
    : protocol + posixResolve(startPath, url);
}
  ...
  if (sourceMap.map && sourceMap.url) {
    ...
    if (originalPosition && originalPosition.source != null)
      return (
        ...
        sourceMap.vite && (originalPosition._vite = !0),
        originalPosition
      );
  }
  return position;
...
function mapEvalOrigin(origin) {
  ...
  return (
    ...
    match ? `eval at ${match[1]} (${mapEvalOrigin(match[2])})` : origin
  );
}
function CallSiteToString() {
  ...
  if (this.isNative()) fileLocation = "native";
    ...
    ((fileName = this.getScriptNameOrSourceURL()),
      !fileName && this.isEval() && ((fileLocation = this.getEvalOrigin()), (fileLocation += ", ")),
      fileName ? (fileLocation += fileName) : (fileLocation += "<anonymous>"));
  ...
  if (this.isToplevel() || isConstructor)
    isConstructor
      ...
      : functionName
        ? (line += functionName)
        : ((line += fileLocation), (addSuffix = !1));
  else {
    let typeName = this.getTypeName();
    typeName === "[object Object]" && (typeName = "null");
    let methodName = this.getMethodName();
    functionName
      ? (typeName && functionName.indexOf(typeName) !== 0 && (line += `${typeName}.`),
        (line += functionName),
        methodName &&
      ...
      (frame.getFunctionName = function () {
        let name =
          state.nextPosition == null
            ...
            : state.nextPosition.name || originalFunctionName();
...
function prepareStackTrace(error, stack) {
  let errorString = `${error.name || "Error"}: ${error.message || ""}`,
    state = {
      nextPosition: null,
      curPosition: null,
    },
  ...
  return (
    (state.curPosition = state.nextPosition = null), errorString + processedStack.reverse().join("")
  );
}
//#endregion
...
function enableSourceMapSupport(runner) {
  if (runner.options.sourcemapInterceptor === "node") {
    ...
    return (
      process.setSourceMapsEnabled(!0), () => !isEnabledAlready && process.setSourceMapsEnabled(!1)
    );
  }
...
function createDefaultImportMeta(modulePath) {
  let href = posixPathToFileHref(modulePath),
    filename = modulePath,
    dirname = posixDirname(modulePath);
  return {
...
function createNodeImportMeta(modulePath) {
  let defaultMeta = createDefaultImportMeta(modulePath),
    href = defaultMeta.url,
    importMetaResolver = createImportMetaResolver();
  return {
    ...defaultMeta,
    main: !1,
    resolve(id, parent) {
...
var ModuleRunner = class {
  ...
  constructor(options, evaluator = new ESModulesEvaluator(), debug) {
    if (
      ...
      if (
        ((this.hmrClient = new HMRClient(
          optionsHmr === !0 || optionsHmr.logger === void 0
            ? hmrLogger
            : optionsHmr.logger === !1
              ? silentConsole
              : optionsHmr.logger,
    ...
    options.sourcemapInterceptor !== !1 &&
  ...
  async cachedRequest(url, mod, callstack = [], metadata) {
    ...
    if ((importee && importers.add(importee), mod.evaluated && mod.promise))
    ...
    try {
      ...
      return (
        (mod.promise = promise),
        (mod.evaluated = !1),
        this.processImport(await promise, meta, metadata)
      );
    } finally {
      ...
      return (
        (this.builtinsPromise ??= (async () => {
          try {
            ...
            let builtins = (await this.transport.invoke("getBuiltins", [])).map((builtin) =>
  ...
  async getModuleInformation(url, importer, cachedModule) {
    ...
    if ("cache" in fetchedModule) {
      if (!cachedModule || !cachedModule.meta)
        throw Error(`Module "${url}" was mistakenly invalidated during fetch phase.`);
      return cachedModule;
    }
    ...
    return (
      "invalidate" in fetchedModule &&
        fetchedModule.invalidate &&
  ...
  async directRequest(url, mod, _callstack) {
    let fetchResult = mod.meta,
      ...
      request = async (dep, metadata) => {
        ...
        return (
          ...
          this.cachedRequest(dep, depMod, callstack, metadata)
        );
      },
      dynamicRequest = async (dep) => (
        (dep = String(dep)),
        dep[0] === "." && (dep = posixResolve(posixDirname(mod.url), dep)),
        request(dep, { isDynamicImport: !0 })
      );
    ...
    let createImportMeta = this.options.createImportMeta ?? createDefaultImportMeta,
      modulePath = cleanUrl(file || moduleId),
      href = posixPathToFileHref(modulePath),
      meta = await createImportMeta(modulePath),
...
function exportAll(exports, sourceModule) {
  if (
    ...
    !(isPrimitive(sourceModule) || Array.isArray(sourceModule) || sourceModule instanceof Promise)
  ) {
    for (let key in sourceModule)
