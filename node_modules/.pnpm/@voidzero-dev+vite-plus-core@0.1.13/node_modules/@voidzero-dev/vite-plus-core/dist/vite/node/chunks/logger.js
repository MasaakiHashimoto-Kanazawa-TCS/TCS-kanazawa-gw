...
const DEFAULT_MAIN_FIELDS = ["browser", "module", "jsnext:main", "jsnext"];
...
const VITE_PACKAGE_DIR = path.join(
  fileURLToPath(
    /** #__KEEP__ */
    import.meta.url,
  ),
...
function createLogger(level = "info", options = {}) {
  ...
  function format(type, msg, options = {}) {
    if (options.timestamp) {
      let tag = "";
      if (type === "info")
        tag = import_picocolors.default.cyan(import_picocolors.default.bold(prefix));
      else if (type === "warn")
        tag = import_picocolors.default.yellow(import_picocolors.default.bold(prefix));
      ...
      if (canClearScreen)
        if (type === lastType && msg === lastMsg) {
          ...
          console[method](
            ...
            import_picocolors.default.yellow(`(x${sameCount + 1})`),
          );
        } else {
          sameCount = 0;
          lastMsg = msg;
...
function printServerUrls(urls, optionsHost, info) {
  const colorUrl = (url) =>
    import_picocolors.default.cyan(
      url.replace(/:(\d+)\//, (_, port) => `:${import_picocolors.default.bold(port)}/`),
    );
  for (const url of urls.local)
    info(
      `  ${import_picocolors.default.green("➜")}  ${import_picocolors.default.bold("Local")}:   ${colorUrl(url)}`,
    );
  for (const url of urls.network)
    info(
      `  ${import_picocolors.default.green("➜")}  ${import_picocolors.default.bold("Network")}: ${colorUrl(url)}`,
    );
  if (urls.network.length === 0 && optionsHost === void 0)
    info(
      import_picocolors.default.dim(
        `  ${import_picocolors.default.green("➜")}  ${import_picocolors.default.bold("Network")}: use `,
      ) +
        import_picocolors.default.bold("--host") +
        import_picocolors.default.dim(" to expose"),
    );
}
//#endregion
export {
  OPTIMIZABLE_ENTRY_RE as A,
  ERR_FILE_NOT_FOUND_IN_OPTIMIZED_DEP_DIR as C,
  JS_TYPES_RE as D,
  FS_PREFIX as E,
  VITE_PLUS_VERSION as F,
  defaultAllowedOrigins as I,
  loopbackHosts as L,
  SPECIAL_QUERY_RE as M,
  VERSION as N,
  KNOWN_ASSET_TYPES as O,
  VITE_PACKAGE_DIR as P,
  wildcardHosts as R,
  ENV_PUBLIC_PATH as S,
  ESBUILD_BASELINE_WIDELY_AVAILABLE_TARGET as T,
  DEFAULT_SERVER_CONDITIONS as _,
  CLIENT_ENTRY as a,
  DEV_PROD_CONDITION as b,
  DEFAULT_ASSETS_INLINE_LIMIT as c,
  DEFAULT_CLIENT_MAIN_FIELDS as d,
  DEFAULT_CONFIG_FILES as f,
  DEFAULT_PREVIEW_PORT as g,
  DEFAULT_EXTERNAL_CONDITIONS as h,
  CLIENT_DIR as i,
  ROLLUP_HOOKS as j,
  METADATA_FILENAME as k,
  DEFAULT_ASSETS_RE as l,
  DEFAULT_EXTENSIONS as m,
  createLogger as n,
  CLIENT_PUBLIC_PATH as o,
  DEFAULT_DEV_PORT as p,
  printServerUrls as r,
  CSS_LANGS_RE as s,
  LogLevels as t,
  DEFAULT_CLIENT_CONDITIONS as u,
  DEFAULT_SERVER_MAIN_FIELDS as v,
  ERR_OPTIMIZE_DEPS_PROCESSING_ERROR as w,
  ENV_ENTRY as x,
  DEP_VERSION_RE as y,
  require_picocolors as z,
};
