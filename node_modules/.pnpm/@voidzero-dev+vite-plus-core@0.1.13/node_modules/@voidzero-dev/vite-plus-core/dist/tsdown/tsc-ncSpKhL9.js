...
function tscEmitBuild(tscOptions) {
  ...
  const project = getOrBuildProjects(context, fsSystem, tsconfig, !incremental, sourcemap).get(
    resolvedId,
  );
  if (!project) {
...
function buildProjects(fsSystem, tsconfig, force, sourcemap) {
  ...
  debug$3(
    `built solution for ${tsconfig} with exit status ${ts
      .createSolutionBuilder(host, [tsconfig], {
        force,
        verbose: true,
      })
...
function initTsMacro() {
  ...
  try {
    ...
    const { proxyCreateProgram } = __require(
      __require.resolve("@volar/typescript", { paths: [tsMacroPath] }),
    );
    const tsMacro = __require(
      __require.resolve("@ts-macro/language-plugin", { paths: [tsMacroPath] }),
    );
    const { getOptions } = __require(
      __require.resolve("@ts-macro/language-plugin/options", { paths: [tsMacroPath] }),
    );
    const getLanguagePlugin = (ts, options) => {
      const $rootDir = options.options.$rootDir;
...
function createProgramFactory(ts, options) {
  ...
  const proxyCreateProgram =
    vueLanguageTools?.proxyCreateProgram || tsMacroLanguageTools?.proxyCreateProgram;
...
function createTsProgramFromParsedConfig({
  ...
  const rootNames = [
    ...new Set([id, ...(entries || parsedConfig.fileNames)].map((f) => fsSystem.resolvePath(f))),
  ];
