...
cli
  ...
  .option("-f, --format <format>", "Bundle format: esm, cjs, iife, umd", { default: "esm" })
  ...
  .option("--platform <platform>", "Target platform", { default: "node" })
  .option("--dts", "Generate dts files")
  .option("--publint", "Enable publint", { default: false })
  .option("--attw", "Enable Are the types wrong integration", { default: false })
  ...
  .action(async (input, flags) => {
    ...
    const { build } = await import("./build-dhDyu66J-C_jUIuMF.js");
    if (input.length > 0) flags.entry = input;
    await build(flags);
  });
cli
  .command("create", '[deprecated] Create a tsdown project. Use "npx create-tsdown" instead.', {
    allowUnknownOptions: true,
  })
  .action(async () => {
    globalLogger.warn(`"tsdown create" is deprecated. Please use "npx create-tsdown" instead.`);
    const { exitCode } = await R(
      "npx",
      ["-y", "create-tsdown@latest", ...process$1.argv.slice(3)],
...
cli
  ...
  .action(async () => {
    ...
    const { exitCode } = await R(
      "npx",
      ["-y", "tsdown-migrate@latest", ...process$1.argv.slice(3)],
      { nodeOptions: { stdio: "inherit" } },
    );
    process$1.exitCode = exitCode;
  });
...
if (lt(process$1.version, "22.18.0"))
  console.warn(
    yellow`[tsdown] Node.js ${process$1.version} is deprecated. Support will be removed in the next minor release. Please upgrade to Node.js 22.18.0 or later.`,
  );
try {
