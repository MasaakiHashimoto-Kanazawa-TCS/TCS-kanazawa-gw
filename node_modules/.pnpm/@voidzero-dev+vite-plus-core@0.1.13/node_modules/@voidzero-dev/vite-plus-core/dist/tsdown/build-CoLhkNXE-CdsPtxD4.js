...
import {
  ...
  t as R,
  u as blue,
  v as importWithError,
  w as resolveRegex,
  x as pkgExists,
  y as matchPattern,
} from "./main-CU_sednE.js";
import { t as createDebug } from "./node-B3Gdtau1.js";
import {
  i as RE_JS,
  n as RE_DTS,
  o as RE_NODE_MODULES$1,
  t as RE_CSS,
} from "./filename-UU7z2A9Z.js";
...
function lowestCommonAncestor(...filepaths) {
  ...
  for (const filepath of rest) {
    ...
    for (const directory of directories)
      if (directory === ancestor[index]) index += 1;
      else {
...
function resolveDepsConfig(config, logger) {
  ...
  if (config.external != null) {
    if (neverBundle != null)
      throw new TypeError("`external` is deprecated. Cannot be used with `deps.neverBundle`.");
  ...
  if (config.noExternal != null) {
    if (alwaysBundle != null)
      throw new TypeError("`noExternal` is deprecated. Cannot be used with `deps.alwaysBundle`.");
  ...
  if (config.inlineOnly != null) {
    if (onlyBundle != null)
      throw new TypeError("`inlineOnly` is deprecated. Cannot be used with `deps.onlyBundle`.");
...
function DepPlugin(
  { pkg, deps: { alwaysBundle, onlyBundle, skipNodeModulesBundle }, logger, nameLabel },
  tsdownBundle,
) {
      ...
      async handler(options, bundle) {
        ...
        for (const chunk of Object.values(bundle)) {
          ...
          for (const id of chunk.moduleIds) {
            ...
            if (!tsdownBundle.inlinedDeps.has(parsed.pkgName))
              tsdownBundle.inlinedDeps.set(parsed.pkgName, /* @__PURE__ */ new Set());
            tsdownBundle.inlinedDeps.get(parsed.pkgName).add(parsed.version);
            const module = this.getModuleInfo(id);
            if (module)
              importers.set(
                parsed.name,
                new Set([...module.importers, ...(importers.get(parsed.name) || [])]),
              );
          }
        }
        ...
        if (onlyBundle) {
          ...
          const unusedPatterns = onlyBundle.filter(
            (pattern) => !Array.from(deps).some((dep) => matchPattern(dep, [pattern])),
          );
...
function normalizePath(path, options) {
  ...
  return convertSlashes(
    path[path.length - 1] !== pathSeparator ? path + pathSeparator : path,
    pathSeparator,
  );
}
...
function getPartialMatcher(patterns, options = {}) {
  ...
  for (let i = 0; i < patternsCount; i++) {
    ...
    for (let j = 0; j < partsCount; j++)
      partMatchers[j] = (0, import_picomatch.default)(parts[j], options);
...
function normalizePattern(pattern, expandDirectories, cwd, props, isIgnore) {
  ...
  if (path$1.isAbsolute(result.replace(ESCAPING_BACKSLASHES, "")))
  ...
  if (
    ...
    while (i < n && parts[i + n] === cwdParts[cwdParts.length + i - n]) {
      result =
        result.slice(0, (n - i - 1) * 3) + result.slice((n - i) * 3 + parts[i + n].length + 1) ||
        ".";
      i++;
    }
  ...
  const fdirOptions = {
    ...
    exclude: options.debug
      ? (_, p) => {
          ...
          const skipped =
            (relativePath !== "." && !partialMatcher(relativePath)) || ignore(relativePath);
...
async function glob(patternsOrOptions, options) {
  ...
  const [crawler, relative] = getCrawler(
    isModern ? patternsOrOptions : patternsOrOptions.patterns,
    opts,
  );
...
function init({ skipNodeModules } = {}) {
  if (process$1.versions.bun)
    throw new Error("init is unnecessary in Bun, use clearRequireCache() instead.");
...
function createConfigCoreLoader(options) {
  ...
  const findConfigs = quansync(function* () {
    ...
    for (const source of sources) {
      ...
      const files = yield findUp(
        source.files.flatMap((file) =>
          !extensions?.length ? [file] : extensions.map((ext) => (ext ? `${file}.${ext}` : file)),
        ),
        {
          cwd,
...
async function cleanOutDir(configs) {
  ...
  for (const config of configs) {
    ...
    for (const file of files)
      if (file.replace(RE_LAST_SLASH, "") !== normalizedOutDir) removes.add(file);
  }
  if (!removes.size) return;
  globalLogger.info(`Cleaning ${removes.size} files`);
  await Promise.all(
    [...removes].map(async (file) => {
      debug$4$2("Removing", file);
      await fsRemove(file);
    }),
  );
  debug$4$2("Removed %d files", removes.size);
}
...
async function cleanChunks(outDir, chunks) {
  await Promise.all(
    chunks.map(async (chunk) => {
      ...
      debug$4$2("Removing chunk file", filePath);
...
async function resolveEntry(logger, entry, cwd, color, nameLabel, root) {
  if (!entry || Object.keys(entry).length === 0) {
    ...
    if (await fsExists(defaultEntry)) entry = { index: defaultEntry };
      ...
      throw new Error(
        `${nameLabel ? `${nameLabel} ` : ""}No input files, try "vp pack <your-file>" or create src/index.ts`,
      );
  }
  ...
  if (entries.length === 0)
    throw new Error(
      `${nameLabel ? `${nameLabel} ` : ""}Cannot find entry: ${JSON.stringify(entry)}`,
    );
  logger.info(
    nameLabel,
...
function isGlobEntry(entry) {
  ...
  if (Array.isArray(entry))
    return entry.some((e) => (typeof e === "string" ? isDynamicPattern(e) : isGlobEntry(e)));
    ...
    (
      await Promise.all(
        Object.entries(entries).map(async ([key, value]) => {
          ...
          if (!files.length)
            throw new Error(
              `Cannot find files for entry key "${key}" with patterns: ${JSON.stringify(patterns)}`,
            );
          let valueGlobBase;
          for (const pattern of patterns) {
          ...
          if (valueGlobBase === void 0)
            throw new Error(
              `Cannot determine base directory for value glob patterns of key "${key}".`,
            );
          return files.map((file) => [
            slash(key.replaceAll("*", stripExtname(path.relative(valueGlobBase, file)))),
...
async function resolveArrayEntry(entries, cwd, root) {
  ...
  for (const e of entries)
    if (typeof e === "string") stringEntries.push(e);
...
function validateSea({ dts, entry, logger, nameLabel }) {
  if (process$1.versions.bun || process$1.versions.deno)
    throw new Error("The `exe` option is not supported in Bun and Deno environments.");
  if (!satisfies$1(process$1.version, ">=25.7.0"))
    throw new Error(
      `Node.js version ${process$1.version} does not support \`exe\` option. Please upgrade to Node.js 25.7.0 or later.`,
    );
  ...
  if (targets?.length) {
    ...
    for (const target of targets) {
      ...
      await buildSingleExe(
        config,
        bundledFile,
        resolveOutputFileName(config.exe, chunk, bundledFile, target, suffix),
        nodeBinaryPath,
        target,
      );
    }
    ...
    await buildSingleExe(
      config,
      bundledFile,
      resolveOutputFileName(config.exe, chunk, bundledFile),
    );
}
function resolveOutputFileName(exe, chunk, bundledFile, target, suffix) {
  ...
  if (target?.platform ? target.platform === "win" : process$1.platform === "win32")
    baseName += ".exe";
  return baseName;
}
async function buildSingleExe(config, bundledFile, outputFile, executable, target) {
  ...
  if ((target?.platform || process$1.platform) === "darwin")
    try {
      await R("codesign", ["--sign", "-", outputPath], {
      ...
      config.logger.warn(
        config.nameLabel,
        `Failed to code-sign the executable. ${process$1.platform === "darwin" ? `You can sign it manually using:\n  codesign --sign - "${outputPath}"` : `Automatic code signing is not supported on ${process$1.platform}.`}`,
      );
    }
...
async function generateExports(pkg, chunks, options, inlinedDeps) {
  ...
  for (const [format, chunksByFormat] of Object.entries(chunks)) {
    ...
    const filteredChunks = chunksByFormat.filter((chunk) => {
      ...
      if (!chunk.isEntry) {
        ...
        if (
          ...
          (chunk.facadeModuleId && RE_NODE_MODULES$1.test(chunk.facadeModuleId))
        )
          return false;
      }
    ...
    const onlyOneEntry =
      filteredChunks.filter((chunk) => !RE_DTS.test(chunk.fileName)).length === 1;
  ...
  let exports = Object.fromEntries(
    sortedExportsMap.map(([name, subExport]) => [name, genSubExport(devExports, subExport)]),
  );
  ...
  if (devExports) {
    publishExports = Object.fromEntries(
      sortedExportsMap.map(([name, subExport]) => [name, genSubExport(false, subExport)]),
    );
...
function getExportName(chunk) {
  ...
  return [name, normalizedName, isDts];
}
...
function resolveTarget(logger, target, color, pkg, nameLabel) {
  ...
  if (targets.length)
    logger.info(nameLabel, `target${targets.length > 1 ? "s" : ""}: ${color(targets.join(", "))}`);
  return targets;
}
...
async function resolveUserConfig(userConfig, inlineConfig) {
  ...
  const [resolvedEntry, resolvedRoot] = await resolveEntry(
    ...
    color,
    nameLabel,
    root ? path.resolve(cwd, root) : void 0,
  );
  ...
  if (publicDir)
    if (copy) throw new TypeError("`publicDir` is deprecated. Cannot be used with `copy`");
  ...
  return (objectFormat ? Object.keys(format) : resolveComma(toArray(format, "esm"))).map(
    (fmt, idx) => {
      const once = idx === 0;
...
function filterEnv(envDict, envPrefixes) {
  ...
  for (const [key, value] of Object.entries(envDict))
    if (value != null && envPrefixes.some((prefix) => key.startsWith(prefix))) env[key] = value;
  return env;
}
  ...
  await Promise.all(
    resolved.map(({ from, to, verbose }) => {
      if (verbose)
        options.logger.info(
          options.nameLabel,
          `Copying files from ${path.relative(options.cwd, from)} to ${path.relative(options.cwd, to)}`,
        );
  ...
  const resolved = (
    await Promise.all(
      copy.map(async (entry) => {
        ...
        if (from.some((f) => isDynamicPattern(f)))
...
async function outputReport(config, chunks, outDir, cjsDts, isDualFormat) {
  ...
  const brotliTextLength = Math.max(
    ...sizes.map((size) => (size.brotliText == null ? 0 : size.brotliText.length)),
  );
  let totalRaw = 0;
  ...
  for (const size of sizes) {
    ...
    config.logger.info(
      config.nameLabel,
      formatLabel,
      dim(outDir + path.sep) + filenameColor((size.isEntry ? bold : noop)(filename)),
...
async function calcSize(options, chunk) {
  ...
  if (raw > options.maxCompressSize)
    debug$5(chunk.fileName, "file size exceeds limit, skip gzip/brotli");
  else {
    if (options.gzip) {
  ...
  return {
    ...
    async buildStart() {
      ...
      if (typeof config.watch !== "boolean")
        for (const file of resolveComma(toArray(config.watch))) this.addWatchFile(file);
  ...
  const configs = (
    await Promise.all(
      rootConfigs.map(async (rootConfig) => {
        const { configs: workspaceConfigs, files: workspaceFiles } = await resolveWorkspace(
          rootConfig,
          inlineConfig,
        );
...
function formatProblem(packageName, problem) {
  ...
  const entrypoint =
    "entrypoint" in problem ? ` at ${slash(path.join(packageName, problem.entrypoint))}` : "";
  switch (problem.kind) {
    case "NoResolution":
    ...
    case "NamedExports": {
      const missingExports =
        problem.missing?.length > 0 ? ` Missing: ${problem.missing.join(", ")}` : "";
...
function mergeInlinedDeps(bundles) {
  ...
  for (const [pkgName, versions] of sorted)
    result[pkgName] = versions.size === 1 ? [...versions][0] : [...versions].toSorted();
  return result;
}
...
function resolveChunkFilename({ outExtensions, fixedExtension, pkg, hash }, inputOptions, format) {
  ...
  return [
    ...
    createChunkFilename(`[name]${suffix}${hash ? "-[hash]" : ""}`, jsExtension, dtsExtension),
  ];
}
...
async function resolveInputOptions(config, format, configFiles, bundle, cjsDts, isDualFormat) {
  ...
  if (report && LogLevels[logger.level] >= 3)
  ...
  return await mergeUserOptions(
    {
      ...
      onLog(level, log, defaultHandler) {
        ...
        if (logger.options?.failOnWarn && level === "warn" && log.code !== "PLUGIN_TIMINGS")
...
function handlePluginInspect(plugins) {
  ...
  else if (typeof plugins === "object" && plugins !== null && "name" in plugins)
    plugins[inspect.custom] = function (depth, options, inspect) {
...
function CssGuardPlugin() {
  return {
    ...
    transform: {
      ...
      handler(_code, id) {
        throw new Error(
          `CSS file "${id}" was encountered but \`@tsdown/css\` is not installed. Please install it: \`npm install @tsdown/css\``,
        );
      },
    },
  };
  ...
  async function onInput(input) {
    ...
    if (input === "h") {
      ...
      for (const shortcut of SHORTCUTS) {
        ...
        globalLogger.info(
          dim`  press ` + bold`${shortcut.key} + enter` + dim` to ${shortcut.description}`,
        );
      }
      return;
    }
  ...
  const bundles = await Promise.all(
    configs.map((options) => {
      return buildSingle(
        options,
        configFiles,
        options.pkg ? configChunksByPkg[options.pkg.packageJsonPath].formats.size > 1 : true,
        clean,
        restart,
        done,
      );
...
async function buildSingle(config, configFiles, isDualFormat, clean, restart, done) {
  ...
  function handleWatcher(watcher) {
    ...
    watcher.on("change", async (id, event) => {
      ...
      if (
        ...
        if (
          Object.keys(config.entry).toSorted().join("\0") !==
          Object.keys(newEntry).toSorted().join("\0")
        ) {
  ...
  async function initBuildOptions() {
    ...
    if (debugRolldownConfigDir)
      await debugBuildOptions(debugRolldownConfigDir, config.name, format, buildOptions);
    const configs = [buildOptions];
    if (format === "cjs" && dts)
      configs.push(await getBuildOptions(config, format, configFiles, bundle, true, isDualFormat));
    return configs;
  }
