...
function resolveOptions(options = {}) {
  ...
  if (!re.existsSync(resolvedOptions.path))
    throw new Error(`[unrun] File not found: ${resolvedOptions.path}`);
  if (!new Set(["none", "jiti", "bundle-require"]).has(resolvedOptions.preset))
    throw new Error(
...
function injectInspectHelper(chunk) {
  ...
  chunk.code = chunk.code.startsWith("#!")
            ...
            for (let i = 0; i < transformedCode.length; ) {
              ...
              if (resolveMatch) {
                ...
                const resolvedUrl = pathToFileURL(
                  path.resolve(path.dirname(normalizedPhysicalId), spec),
                ).href;
...
async function unrun(options) {
  ...
  try {
    ...
    throw new Error(
      `[unrun] Import failed (code length: ${output.chunk.code.length}): ${error.message}`,
      { cause: error },
    );
  }
  return {
