...
function getCodeFrame(source, line, column) {
  ...
  const maxLineLength = Math.max(
    tabsToSpaces(lines[line - 1].slice(0, column)).length + MIN_CHARACTERS_SHOWN_AFTER_LOCATION + 3,
  ...
  return lines
    .map((sourceLine, index) => {
      ...
      if (displayedLine.length > maxLineLength)
        displayedLine = `${displayedLine.slice(0, maxLineLength - 3)}${ELLIPSIS}`;
      if (isErrorLine) {
        const indicator =
...
//#endregion
export {
  logInvalidLogPosition as a,
  logPluginError as c,
  logInputHookInOutputPlugin as i,
  locate as l,
  error as n,
  logMultipleWatcherOption as o,
  logCycleLoading as r,
  logParseError as s,
  augmentCodeLocation as t,
  getCodeFrame as u,
};
