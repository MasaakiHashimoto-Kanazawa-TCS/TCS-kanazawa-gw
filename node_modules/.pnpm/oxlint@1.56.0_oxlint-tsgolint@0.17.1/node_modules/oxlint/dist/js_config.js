...
const isObject = (v) => typeof v == "object" && !!v && !ArrayIsArray(v),
  VITE_OXLINT_CONFIG_FIELD = "lint";
function validateConfigExtends(root) {
  let visited = /* @__PURE__ */ new WeakSet(),
    inStack = /* @__PURE__ */ new WeakSet(),
    stackObjects = [],
    stackPaths = [],
    formatCycleError = (refPath, cycleStart, idx) => `\`extends\` contains a circular reference.

${refPath} points back to ${cycleStart}\nCycle: ${idx === -1 ? `${cycleStart} -> ${cycleStart}` : [...stackPaths.slice(idx), cycleStart].join(" -> ")}`,
    visit = (config, path) => {
      if (visited.has(config)) return;
      if (inStack.has(config)) {
        let idx = stackObjects.indexOf(config),
          cycleStart = idx === -1 ? "<unknown>" : stackPaths[idx];
      ...
      if (maybeExtends !== void 0) {
        ...
        for (let i = 0; i < maybeExtends.length; i++) {
          ...
          if (inStack.has(item)) {
            let idx = stackObjects.indexOf(item),
              cycleStart = idx === -1 ? "<unknown>" : stackPaths[idx];
...
async function loadJsConfigs(paths) {
  try {
    let cacheKey = DateNow(),
      results = await Promise.allSettled(
        paths.map(async (path) => {
          ...
          if (!isObject(config))
            throw Error("Configuration file must have a default export that is an object.");
          return (
    ...
    return errors.length > 0
      ? JSONStringify({ Failures: errors })
      : JSONStringify({ Success: successes });
  } catch (err) {
