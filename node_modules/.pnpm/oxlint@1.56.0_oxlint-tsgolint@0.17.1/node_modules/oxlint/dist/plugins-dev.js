import {
  ...
  c as allOptions,
  d as diagnostics,
  f as replacePlaceholders,
  g as BUFFER_ALIGN,
  h as ACTIVE_SIZE,
  i as resetStateAfterError,
  l as setOptions,
  m as getNodeByRangeIndex,
  o as registerPlugin,
  p as getLineColumnFromOffset,
  r as lintFileImpl,
  s as registeredRules,
  t as buffers,
  u as PLACEHOLDER_REGEX,
  y as __commonJSMin,
} from "./lint.js";
import {
  _ as ObjectValues,
  ...
  p as ObjectHasOwn,
  t as ArrayFrom,
  u as ObjectDefineProperty,
} from "./globals.js";
import {
  a as rawTransferSupported$1,
  i as parseRawSync,
  n as getBufferOffset,
  t as applyFixes,
} from "./bindings.js";
...
var import_json_stable_stringify_without_jsonify = /* @__PURE__ */ __toESM(
  /* @__PURE__ */ __commonJSMin((exports, module) => {
    module.exports = function (obj, opts) {
      ((opts ||= {}), typeof opts == "function" && (opts = { cmp: opts }));
      var space = opts.space || "";
      typeof space == "number" && (space = Array(space + 1).join(" "));
      var cycles = typeof opts.cycles == "boolean" ? opts.cycles : !1,
        replacer =
          opts.replacer ||
          function (key, value) {
            return value;
      ...
      return (function stringify(parent, key, node, level) {
        ...
        if (
          ...
          if (isArray(node)) {
            ...
            for (
              var keys = objectKeys(node).sort(cmp && cmp(node)), out = [], i = 0;
              i < keys.length;
              i++
            ) {
              var key = keys[i],
...
function parse(path, sourceText, options) {
  ...
  let sourceStartPos = ACTIVE_SIZE - maxSourceByteLen,
    sourceBuffer = new Uint8Array(
      buffer.buffer,
      buffer.byteOffset + sourceStartPos,
      maxSourceByteLen,
    ),
    { read, written: sourceByteLen } = textEncoder.encodeInto(sourceText, sourceBuffer);
 ...
 */
function initBuffer() {
  let arrayBuffer = new ArrayBuffer(ARRAY_BUFFER_SIZE),
    offset = getBufferOffset(new Uint8Array(arrayBuffer));
  ((buffer = new Uint8Array(arrayBuffer, offset, BUFFER_SIZE)),
...
function rawTransferSupported() {
  return (
    rawTransferIsSupported === null &&
      (rawTransferIsSupported = rawTransferRuntimeSupported() && rawTransferSupported$1()),
...
let it = typeof globalObj.it == "function" ? globalObj.it : defaultIt,
  itOnly = it !== defaultIt && typeof it.only == "function" ? it.only.bind(it) : null;
/**
 * Get `it` function.
...
function getItOnly() {
  if (itOnly === null)
    throw Error(
      "To use `only`, use `RuleTester` with a test framework that provides `it.only()` like Mocha, or provide a custom `it.only` function by assigning it to `RuleTester.itOnly`",
    );
  return itOnly;
}
...
var RuleTester = class {
  ...
  constructor(config) {
    if (config === void 0) config = null;
      ...
      throw TypeError("`config` must be an object if provided");
    this.#config = config;
  }
  /**
  ...
  run(ruleName, rule, tests) {
    ...
    describe(ruleName, () => {
      (tests.valid.length > 0 &&
        describe("valid", () => {
          let seenTestCases = /* @__PURE__ */ new Set();
          for (let test of tests.valid)
            (typeof test == "string" && (test = { code: test }),
              getIt(test.only)(getTestName(test), () => {
...
function assertValidTestCasePasses(test, plugin, config) {
  ((test = mergeConfigIntoTestCase(test, config)),
    assertErrorCountIsCorrect(lint(test, plugin), 0));
}
/**
...
function assertInvalidTestCasePasses(test, plugin, config) {
  test = mergeConfigIntoTestCase(test, config);
  let diagnostics = lint(test, plugin),
    { errors } = test;
      ...
      typeof error == "string" || error instanceof RegExp
        ? (assertMessageMatches(diagnostic.message, error),
          assert(
            ...
            `Error at index ${errorIndex} has suggestions. Please convert the test error into an object and specify \`suggestions\` property on it to test suggestions`,
          ))
        : (assertInvalidTestCaseMessageIsCorrect(diagnostic, error, messages),
          ...
          ObjectHasOwn(error, "suggestions") &&
  ...
  let { code } = test,
    eslintCompat = test.eslintCompat === !0,
    fixedCode = runFixes(diagnostics, code, eslintCompat);
  fixedCode === null && (fixedCode = code);
  let { recursive } = test,
    extraPassCount = typeof recursive == "number" ? recursive : recursive === !0 ? 10 : 0;
  if (extraPassCount > 0 && fixedCode !== code)
    for (let pass = 0; pass < extraPassCount; pass++) {
      let newFixedCode = runFixes(
        lint(
          {
            ...test,
            code: fixedCode,
...
function assertInvalidTestCaseMessageIsCorrect(diagnostic, error, messages) {
  if (ObjectHasOwn(error, "message")) {
    (assert(
      !ObjectHasOwn(error, "messageId"),
      "Error should not specify both `message` and a `messageId`",
    ),
      assert(!ObjectHasOwn(error, "data"), "Error should not specify both `data` and `message`"),
...
function assertMessageIdIsCorrect(
  ...
  if (
    ...
    !ObjectHasOwn(messages, messageId))
  ) {
    let legalMessageIds = `[${ObjectKeys(messages)
      .map((key) => `'${key}'`)
      .join(", ")}]`;
  ...
  let ruleMessage = messages[messageId],
    unsubstitutedPlaceholders = getUnsubstitutedMessagePlaceholders(
      reportedMessage,
      ruleMessage,
      data,
    );
    ...
    (unsubstitutedPlaceholders.length !== 0 &&
      assert.fail(
        `${prefix}The reported message has ` +
          ...
          `. Please provide the missing ${unsubstitutedPlaceholders.length > 1 ? "values" : "value"} via the \`data\` property.`,
      ),
    data !== void 0)
  ) {
...
function assertInvalidTestCaseLocationIsCorrect(diagnostic, error, test) {
  let actualLocation = {},
    expectedLocation = {},
    columnOffset = test.eslintCompat === !0 ? 1 : 0;
  (ObjectHasOwn(error, "line") &&
    ((actualLocation.line = diagnostic.line), (expectedLocation.line = error.line)),
    ObjectHasOwn(error, "column") &&
      ((actualLocation.column = diagnostic.column + columnOffset),
      (expectedLocation.column = error.column)));
  let canVoidEndLocation =
    test.eslintCompat === !0 &&
    diagnostic.endLine === diagnostic.line &&
    diagnostic.endColumn === diagnostic.column;
  (ObjectHasOwn(error, "endLine") &&
    (error.endLine === void 0 && canVoidEndLocation
      ? (actualLocation.endLine = void 0)
      : (actualLocation.endLine = diagnostic.endLine),
    (expectedLocation.endLine = error.endLine)),
    ObjectHasOwn(error, "endColumn") &&
      (error.endColumn === void 0 && canVoidEndLocation
        ? (actualLocation.endColumn = void 0)
        : (actualLocation.endColumn = diagnostic.endColumn + columnOffset),
      (expectedLocation.endColumn = error.endColumn)),
    ObjectKeys(expectedLocation).length > 0 &&
...
function assertSuggestionsAreCorrect(diagnostic, error, messages, test) {
  let actualSuggestions = diagnostic.suggestions ?? [],
    expectedSuggestions = error.suggestions;
  assert.strictEqual(
    actualSuggestions.length,
    expectedSuggestions.length,
    `Error should have ${expectedSuggestions.length} suggestion${expectedSuggestions.length > 1 ? "s" : ""}. Instead found ${actualSuggestions.length} suggestion${actualSuggestions.length > 1 ? "s" : ""}.`,
  );
  let eslintCompat = test.eslintCompat === !0;
  for (let i = 0; i < expectedSuggestions.length; i++) {
    let actual = actualSuggestions[i],
      expected = expectedSuggestions[i],
      prefix = `Suggestion at index ${i}`;
    (assertSuggestionMessageIsCorrect(actual, expected, messages, prefix),
      assert(ObjectHasOwn(expected, "output"), `${prefix}: \`output\` property is required`));
...
function assertSuggestionMessageIsCorrect(actual, expected, messages, prefix) {
  if (ObjectHasOwn(expected, "desc")) {
    (assert(
      !ObjectHasOwn(expected, "messageId"),
      `${prefix}: Test should not specify both \`desc\` and \`messageId\``,
    ),
      assert(
        !ObjectHasOwn(expected, "data"),
...
function assertErrorCountIsCorrect(diagnostics, expectedErrorCount) {
  diagnostics.length !== expectedErrorCount &&
    assert.strictEqual(
      diagnostics.length,
      expectedErrorCount,
      util.format(
...
function getUnsubstitutedMessagePlaceholders(message, raw, data) {
  ...
  let known = getMessagePlaceholders(raw),
    provided = data === void 0 ? [] : ObjectKeys(data);
...
function createConfigForRun(config) {
  ...
  return (
    ...
    config !== null && addConfigPropsFrom(config, merged),
    merged
  );
}
function addConfigPropsFrom(config, merged) {
  for (let key of ObjectKeys(config))
    TEST_CASE_PROP_KEYS.has(key) ||
      (key === "languageOptions"
        ? (merged.languageOptions = mergeLanguageOptions(
...
function mergeLanguageOptions(localLanguageOptions, baseLanguageOptions) {
  return localLanguageOptions == null
    ? (baseLanguageOptions ?? void 0)
    : baseLanguageOptions == null
...
function mergeParserOptions(localParserOptions, baseParserOptions) {
  return localParserOptions == null
    ? (baseParserOptions ?? void 0)
    : baseParserOptions == null
      ? localParserOptions
      : {
          ...baseParserOptions,
          ...localParserOptions,
...
function mergeEcmaFeatures(localEcmaFeatures, baseEcmaFeatures) {
  return localEcmaFeatures == null
    ? (baseEcmaFeatures ?? void 0)
    : baseEcmaFeatures == null
      ? localEcmaFeatures
      : {
          ...baseEcmaFeatures,
          ...localEcmaFeatures,
...
function lint(test, plugin) {
  let parseOptions = getParseOptions(test),
    path,
    { filename, cwd } = test;
  ...
  try {
    ...
    let globalsJSON = getGlobalsJson(test),
      settingsJSON = JSONStringify(test.settings ?? {});
    ...
    ((registeredRules.length = 0),
      allOptions !== null && (allOptions.length = 1),
...
function getParseOptions(test) {
  ...
  if (((languageOptions ??= EMPTY_LANGUAGE_OPTIONS), languageOptions.parser != null))
    throw Error("Custom parsers are not supported");
  let { sourceType } = languageOptions;
  if (sourceType != null) {
    if (test.eslintCompat === !0 && sourceType === "unambiguous")
      throw Error(
  ...
  if (
    ...
    lang == null
      ? parserOptions.ecmaFeatures?.jsx === !0 && (parseOptions.lang = "jsx")
...
function getGlobalsJson(test) {
  ...
  let originalEnvs = test.languageOptions?.env,
    envs = {};
  if (originalEnvs != null)
    for (let [key, value] of ObjectEntries(originalEnvs))
      value !== !1 &&
        ObjectDefineProperty(envs, key, {
          value: !0,
          writable: !0,
          enumerable: !0,
          configurable: !0,
...
function assertValidTestCaseIsWellFormed(test, seenTestCases) {
  (assertTestCaseCommonPropertiesAreWellFormed(test),
    assert(
      !("errors" in test) || test.errors === void 0,
      "Valid test case must not have `errors` property",
    ),
    assert(
      !("output" in test) || test.output === void 0,
...
function assertInvalidTestCaseIsWellFormed(test, seenTestCases, ruleName) {
  ...
  (typeof errors == "number"
    ...
    : (assert(
      ...
      assert(
        ArrayIsArray(errors),
        `Invalid 'errors' property for invalid test of rule \`${ruleName}\`:expected a number or an array but got ${errors === null ? "null" : typeof errors}`,
      ),
      assert(errors.length !== 0, "Invalid cases must have at least one error")),
    ObjectHasOwn(test, "output") &&
      assert(
...
function assertNotDuplicateTestCase(test, seenTestCases) {
  ...
  let serializedTestCase = (0, import_json_stable_stringify_without_jsonify.default)(test, {
    replacer(key, value) {
...
function isSerializable(value, seenObjects = /* @__PURE__ */ new Set()) {
  ...
  for (let property in value) {
    ...
    if (
      !isSerializablePrimitiveOrPlainObject(prop) ||
      (!(typeof prop != "object" || !prop) &&
        !isSerializable(prop, new Set([...seenObjects, value])))
    )
      return !1;
  }
  return !0;
...
function isSerializablePrimitiveOrPlainObject(value) {
  return (
    ...
    typeof value == "number" ||
    (typeof value == "object" && (value.constructor === Object || ArrayIsArray(value)))
  );
}
//#endregion
export { RuleTester };
