...
function createWorkspaceWrapper(workspace) {
  return createWorkspace === null
    ? import("./workspace.js").then(
        (mod) => (({ createWorkspace, destroyWorkspace } = mod), createWorkspace(workspace)),
      )
    : Promise.resolve(createWorkspace(workspace));
}
/**
 * Destroy a workspace.
 *
...
function loadJsConfigsWrapper(paths) {
  return loadJsConfigs === null
    ? import("./js_config.js").then(
        (mod) => ((loadJsConfigs = mod.loadJsConfigs), loadJsConfigs(paths)),
      )
    : loadJsConfigs(paths);
}
const args = process.argv.slice(2);
(process.stdout.isTTY ||
  (process.stdin._handle?.setBlocking?.(!0), process.stdout._handle?.setBlocking?.(!0)),
  args.includes("--lsp") && (process.stdout.write = process.stderr.write.bind(process.stderr)),
  (await lint(
    args,
    loadPluginWrapper,
