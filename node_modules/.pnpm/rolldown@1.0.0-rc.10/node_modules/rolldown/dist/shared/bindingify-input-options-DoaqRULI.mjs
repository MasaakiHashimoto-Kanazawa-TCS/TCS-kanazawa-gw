...
import {
  a as logInvalidLogPosition,
  c as logPluginError,
  n as error,
  r as logCycleLoading,
  t as augmentCodeLocation,
} from "./logs-D80CXhvg.mjs";
import {
  i as bindingifyManifestPlugin,
  n as BuiltinPlugin,
  r as bindingifyBuiltInPlugin,
  t as normalizedStringOrRegex,
} from "./normalize-string-or-regex-D7wlw16t.mjs";
import { a as unreachable, o as unsupported, r as noop, t as arraify } from "./misc-DJYbNKZX.mjs";
import {
  a as bindingifySourcemap,
  i as unwrapBindingResult,
  t as aggregateBindingErrorsIntoJsError,
} from "./error-BO4SlZV_.mjs";
...
function lazyProp(target, propertyKey, descriptor) {
  if (!target.constructor[LAZY_FIELDS_KEY])
    target.constructor[LAZY_FIELDS_KEY] = /* @__PURE__ */ new Map();
...
function normalizeTransformHookSourcemap(id, originalCode, rawMap) {
  ...
  if (
    ...
    (map.sources && map.sources.length === 1 && map.sources[0] !== id)
  )
    map.sources = [id];
  return map;
}
...
function transformFilterMatcherToFilterExprs(filterOption) {
  ...
  let codeExcludes = [];
  if (id)
    [idIncludes, idExcludes] = t(
      generalHookFilterMatcherToFilterExprs(id, "id") ?? [],
      (m) => m.kind === "include",
    );
  if (code)
    [codeIncludes, codeExcludes] = t(
...
function bindingPluginOrder(order) {
  switch (order) {
    case "post":
      return import_binding.BindingPluginOrder.Post;
    case "pre":
      return import_binding.BindingPluginOrder.Pre;
    case null:
    case void 0:
      return;
    default:
  ...
  async load(options) {
    ...
    if (!loadPromise) {
      loadPromise = this.context
        .load(id, options.moduleSideEffects ?? void 0, options.packageJsonPath ?? void 0)
        .catch(() => {
...
function bindingifyBuildEnd(args) {
  ...
  return {
    plugin: async (ctx, err) => {
      await handler.call(
        createPluginContext(args, ctx),
        err ? aggregateBindingErrorsIntoJsError(err) : void 0,
      );
    },
    meta: bindingifyPluginHookMeta(meta),
  };
...
function bindingifyAttachDebugInfo(attachDebugInfo) {
  switch (attachDebugInfo) {
    case void 0:
      return;
    case "full":
      return import_binding.BindingAttachDebugInfo.Full;
    case "simple":
      return import_binding.BindingAttachDebugInfo.Simple;
    case "none":
      return import_binding.BindingAttachDebugInfo.None;
  }
}
...
function bindingifyLogLevel(logLevel) {
  switch (logLevel) {
    case "silent":
      return import_binding.BindingLogLevel.Silent;
    case "debug":
      return import_binding.BindingLogLevel.Debug;
    case "warn":
      return import_binding.BindingLogLevel.Warn;
    case "info":
      return import_binding.BindingLogLevel.Info;
    default:
  ...
  switch (config.propertyReadSideEffects) {
    case "always":
      normalizedConfig.propertyReadSideEffects =
        import_binding.BindingPropertyReadSideEffects.Always;
      break;
    case false:
      normalizedConfig.propertyReadSideEffects =
        import_binding.BindingPropertyReadSideEffects.False;
      break;
    default:
  }
  switch (config.propertyWriteSideEffects) {
    case "always":
      normalizedConfig.propertyWriteSideEffects =
        import_binding.BindingPropertyWriteSideEffects.Always;
      break;
    case false:
      normalizedConfig.propertyWriteSideEffects =
        import_binding.BindingPropertyWriteSideEffects.False;
      break;
    default:
  }
...
export {
  ...
  description as S,
  LOG_LEVEL_INFO as _,
  RolldownMagicString as a,
  RUNTIME_MODULE_ID as b,
  __decorate as c,
  PlainObjectLike as d,
  MinimalPluginContextImpl as f,
  LOG_LEVEL_ERROR as g,
  LOG_LEVEL_DEBUG as h,
  transformModuleInfo as i,
  transformAssetSource as l,
  normalizeLog as m,
  PluginContextData as n,
  transformToRollupOutput as o,
  normalizeHook as p,
  bindingifyPlugin as r,
  transformRenderedChunk as s,
  bindingifyInputOptions as t,
  lazyProp as u,
  LOG_LEVEL_WARN as v,
  VERSION as x,
  logLevelPriority as y,
};
