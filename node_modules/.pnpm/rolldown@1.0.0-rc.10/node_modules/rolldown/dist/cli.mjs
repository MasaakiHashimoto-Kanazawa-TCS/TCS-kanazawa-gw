...
import {
  a as getInputCliKeys,
  i as getCliSchemaInfo,
  l as styleText,
  o as getOutputCliKeys,
  r as logger,
  s as validateCliOptions,
...
for (const config of Object.values(alias))
  if (config?.abbreviation) shortAliases.add(config.abbreviation);
function parseCliArguments() {
  ...
  if (unknownKeys.length > 0) {
    ...
    logger.warn(
      `Option \`${unknownKeys.join(",")}\` ${single ? "is" : "are"} unrecognized. We will ignore ${single ? "this" : "those"} option${single ? "" : "s"}.`,
    );
  }
...
async function bundleWithCliOptions(cliOptions) {
  try {
    ...
    const { output: outputs } = await _usingCtx$1
      .a(await rolldown(cliOptions.input))
      .generate(cliOptions.output);
    if (outputs.length === 0) {
    ...
    switch (event.code) {
      ...
      case "BUNDLE_START":
        if (changedFile.length > 0)
          logger.log(
            `Found ${styleText("bold", changedFile.map(relativeId).join(", "))} changed, rebuilding...`,
          );
      ...
      case "BUNDLE_END":
        ...
        logger.success(
          `Rebuilt ${styleText("bold", relativeId(event.output[0]))} in ${styleText("green", ms(event.duration))}.`,
        );
        break;
      case "ERROR":
        await event.result.close();
...
if (!checkNodeVersion(process$1.versions.node))
  logger.warn(
    `You are using Node.js ${process$1.versions.node}. Rolldown requires Node.js version 20.19+ or 22.12+. Please upgrade your Node.js version.`,
  );
async function main() {
