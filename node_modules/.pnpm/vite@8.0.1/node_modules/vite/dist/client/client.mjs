...
const cspNonce =
  "document" in globalThis ? document.querySelector("meta[property=csp-nonce]")?.nonce : void 0;
  ...
  text(selector, text, linkFiles = false) {
    ...
    if (!linkFiles) el.textContent = text;
      ...
      while ((match = fileRE.exec(text))) {
        ...
        link.onclick = () => {
          fetch(
            new URL(`${base$1}__open-in-editor?file=${encodeURIComponent(file)}`, import.meta.url),
          );
        };
        el.appendChild(link);
...
if (customElements && !customElements.get("vite-error-overlay"))
...
if (isBundleMode && typeof DevRuntime !== "undefined") {
  ...
  (_ref = globalThis).__rolldown_runtime__ ??
    (_ref.__rolldown_runtime__ = new ViteDevRuntime({
      send(message) {
        switch (message.type) {
