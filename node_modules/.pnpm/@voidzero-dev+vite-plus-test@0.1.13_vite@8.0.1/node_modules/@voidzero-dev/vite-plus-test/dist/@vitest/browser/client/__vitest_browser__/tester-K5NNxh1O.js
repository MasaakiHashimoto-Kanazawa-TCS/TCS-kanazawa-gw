import {
  ...
  m as moduleRunner,
  d as getWorkerState,
  g as getBrowserState,
  e as getTestName,
  a as getConfig,
} from "./utils-C2ISqq1C.js";
import { onCancel, globalChannel, channel, client } from "../../client.js";
import { userEvent, page, server } from "vitest/browser";
import {
  getSafeTimers,
  DecodedMap as DecodedMap$1,
  getOriginalPosition as getOriginalPosition$1,
  loadDiffConfig,
  ...
  return class BrowserTestRunner extends runnerClass {
      ...
      await this.commands.triggerCommand("__vitest_markTrace", [
        {
          ...
          stack:
            (_c = (_b = test.result) == null ? void 0 : _b.errors) == null ? void 0 : _c[0].stack,
        },
      ]);
      const name = getTraceName(test, retry, repeats);
    ...
    onAfterRunTask = async (task) => {
      ...
      if (traces2.length) {
        if (
          ...
          ((_b = task.result) == null ? void 0 : _b.state) === "pass"
        ) {
          await this.commands.triggerCommand("__vitest_deleteTracing", [{ traces: traces2 }]);
        } else {
    ...
    onTaskFinished = async (task) => {
      ...
      if (
        ...
        ((_a = task.result) == null ? void 0 : _a.state) === "fail" &&
        task.type === "test" &&
        task.artifacts.every((artifact) => artifact.type !== "internal:toMatchScreenshot")
      ) {
        const screenshot = await page
          .screenshot(
            {
              timeout:
                ((_b = this.config.browser.providerOptions) == null ? void 0 : _b.actionTimeout) ??
                5e3,
            },
            /** TODO */
          )
    ...
    onAfterRunFiles = async (files) => {
      ...
      const [coverage] = await Promise.all([
        (_a = coverageModule == null ? void 0 : coverageModule.takeCoverage) == null
          ? void 0
          : _a.call(coverageModule),
        mocker.invalidate(),
    ...
    onTestAnnotate = (test, annotation) => {
      ...
      return this.onTestArtifactRecord(test, artifact).then(
...
function setupConsoleLogSpy() {
  ...
  console$1.trace = (...args) => {
    ...
    const processor =
      ((_a = globalThis.__vitest_worker__) == null ? void 0 : _a.onFilterStackTrace) ||
      ((s) => s || "");
...
function sendLog(type, content, disableStack) {
  ...
  const taskId =
    ((_b = (_a = globalThis.__vitest_worker__) == null ? void 0 : _a.current) == null
      ? void 0
      : _b.id) ?? unknownTestId;
  const origin =
    getConfig().printConsoleTrace && !disableStack
      ? (_c = new Error("STACK_TRACE").stack) == null
        ? void 0
        : _c.split("\n").slice(1).join("\n")
      : void 0;
...
class MockerRegistry {
  ...
  register(typeOrEvent, raw, id, url2, factoryOrRedirect) {
    ...
    if (type === "manual") {
      ...
      const mock =
        type === "automock"
          ? new AutomockedModule(raw, id, url2)
          : new AutospiedModule(raw, id, url2);
      this.add(mock);
      return mock;
...
class ManualMockedModule {
  ...
  resolve() {
    ...
    if (
      typeof exports$1 === "object" &&
      typeof (exports$1 == null ? void 0 : exports$1.then) === "function"
    ) {
...
class ModuleMocker {
  ...
  async importActual(id, importer) {
    ...
    return this.wrapDynamicImport(
      ...
      const m = mod.default;
      return (m == null ? void 0 : m.__esModule)
        ? m
        : {
  ...
  async importMock(rawId, importer) {
    ...
    if (mock.type === "automock" || mock.type === "autospy") {
      ...
      const moduleObject = await __vitePreload(
        () =>
          import(
            /* @vite-ignore */
      ...
      .resolveMock(rawId, importer, {
        mock:
          typeof factoryOrOptions === "function"
            ? "factory"
            : (factoryOrOptions == null ? void 0 : factoryOrOptions.spy)
              ? "spy"
              : "auto",
      })
  ...
  async triggerCommand(command, args, clientError = new Error("empty")) {
    ...
    return traces2.$(
      ...
      async () => {
        var _a2;
        if (shouldMarkTrace) {
          await rpc2.triggerCommand(sessionId, "__vitest_groupTraceStart", filepath, [
            {
        ...
        try {
          ...
          clientError.stack =
            (_a2 = clientError.stack) == null
              ? void 0
              : _a2.replace(clientError.message, err.message);
          throw clientError;
        } finally {
          if (shouldMarkTrace) {
            await rpc2.triggerCommand(sessionId, "__vitest_groupTraceEnd", filepath, []);
          }
        }
      },
    );
...
const traces = new Traces({
  enabled: !!(
    (otelConfig == null ? void 0 : otelConfig.enabled) &&
    (otelConfig == null ? void 0 : otelConfig.browserSdkPath)
  ),
...
async function prepareTestEnvironment(options) {
  ...
  const mocker = new VitestBrowserClientMocker(interceptor, rpc2, SpyModule.createMockInstance, {
...
async function prepare(options) {
  ...
  await Promise.all([
    ...
    (async () => {
      const VitestIndex = await __vitePreload(
        () => import("../../../../index.js"),
        true ? [] : void 0,
      );
...
async function cleanup() {
  ...
  await stopCoverageInsideWorker(config.coverage, moduleRunner, {
