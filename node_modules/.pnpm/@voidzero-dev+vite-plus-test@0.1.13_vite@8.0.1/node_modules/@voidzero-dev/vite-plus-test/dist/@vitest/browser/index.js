...
import { extractSourcemapFromFile } from "../utils/source-map/node.js";
import { resolve as resolve$1, basename as basename$1, dirname as dirname$1 } from "node:path";
import { platform } from "node:os";
import { PNG } from "pngjs";
import { diff } from "../../vendor/blazediff_core.mjs";
...
async function getBrowserProvider(options, project) {
  ...
  if (supportedBrowsers.length && !supportedBrowsers.includes(browser)) {
    throw new Error(
      `${name}Browser "${browser}" is not supported by the browser provider "${options.provider.name}". Supported browsers: ${supportedBrowsers.join(", ")}.`,
    );
  }
...
function BrowserContext(globalServer) {
  return {
    ...
    resolveId(id, importer) {
      if (
        id === ID_CONTEXT ||
        id === "@voidzero-dev/vite-plus-test/browser" ||
        id === "vite-plus/test/browser"
      ) {
  ...
  return [
    {
      ...
      resolveId(id) {
        ...
        const vitestSubpathMap = {
          ...
          "vitest/browser-webdriverio": resolve(
            packageRoot,
            "@vitest/browser-webdriverio/index.js",
          ),
    ...
    },
    {
      ...
      async config() {
        ...
        const include = [];
        const provider =
          parentServer.config.browser.provider || [...parentServer.children][0]?.provider;
...
const pixelmatch = (reference, actual, { createDiff, ...options }) => {
  ...
  return {
    ...
    message: pass
      ? null
      : `${mismatchedPixels} pixels (ratio ${(Math.ceil((mismatchedPixels / imageArea) * 100) / 100).toFixed(2)}) differ.`,
  };
};
...
async function getStableScreenshot(
  { codec, context, comparator, comparatorOptions, element, name, reference, screenshotOptions },
  signal,
) {
...
class ProjectBrowser {
  ...
  constructor(parent, project, base) {
    ...
    this.testerHtml = readFile$1(this.testerFilepath, "utf8").then(
...
class ParentBrowserProject {
  ...
  constructor(project, base) {
    ...
    this.injectorJs = readFile$1(resolve(distRoot, "client/esm-client-injector.js"), "utf8").then(
...
function createBirpc($functions, options) {
  ...
  const builtinMethods = {
    ...
    $callRaw: (options$1) =>
      _call(options$1.method, options$1.args, options$1.event, options$1.optional),
    $rejectPendingCalls,
    get $closed() {
      return $closed;
    },
...
function hasGlobalAriaAttribute(element, forRole) {
  return [...kGlobalAriaAttributes].some(([attr, prohibited]) => {
    return (
      !(prohibited == null ? void 0 : prohibited.has(forRole || "")) && element.hasAttribute(attr)
    );
  });
}
...
function shouldSkipForTextMatching(element) {
  ...
  return (
    ...
    (document2.head && document2.head.contains(element))
  );
}
...
function innerAsLocators(factory, parsed, isFrameLocator = false, maxOutputSize = 20) {
  ...
  for (let index = 0; index < parts.length - 1; index++) {
    if (
      ...
      parts[index + 1].body === "enter-frame"
    ) {
...
function generateSelectorFor(injectedScript, targetElement, options) {
  ...
  const calculate = (element, allowText) => {
    ...
    const noTextCandidates = buildNoTextCandidates(injectedScript, element, options).map(
      (token) => [token],
    );
...
var _Ivya = class _Ivya {
  ...
  queryAllByTestId(text, container = document.documentElement) {
    const selector = this.parseSelector(getByTestIdSelector(_Ivya.options.testIdAttribute, text));
    return this.querySelectorAll(selector, container);
  }
        ...
        if (msg.includes("optimized dependencies changed. reloading")) {
          logger.warn(
            [
              ...
              c.yellow(
                `For a stable experience, please add mentioned dependencies to your config's ${c.bold("`optimizeDeps.include`")} field manually.\n\n`,
              ),
            ].join(""),
          );
        }
