...
let A$1;
const B$1 = 1 === new Uint8Array(new Uint16Array([1]).buffer)[0];
function parse$1(E, g = "@") {
  if (!A$1) throw new Error("Not initialized");
  const w = E.length + 1,
    D = (A$1.__heap_base.value || A$1.__heap_base) + 4 * w - A$1.memory.buffer.byteLength;
  D > 0 && A$1.memory.grow(Math.ceil(D / 65536));
  const G = A$1.sa(w);
  (B$1 ? C$1 : I)(E, new Uint16Array(A$1.memory.buffer, G, w));
  const S = A$1.parseCJS(G, E.length, 0, 0, 0);
  if (S) {
    const B = new Error(
      `Parse error ${g}${A$1.e()}:${E.slice(0, A$1.e()).split("\n").length}:${A$1.e() - E.lastIndexOf("\n", A$1.e() - 1)}`,
    );
    throw (
      Object.assign(B, { idx: A$1.e() }),
      (5 !== S && 6 !== S && 7 !== S) || Object.assign(B, { code: "ERR_LEXER_ESM_SYNTAX" }),
      B
    );
  }
  let o = new Set(),
    ...
    y = new Set();
  for (; A$1.rre(); ) {
    const B = Q$1(E.slice(A$1.res(), A$1.ree()));
    B && R.add(B);
  }
  for (; A$1.ru(); ) y.add(Q$1(E.slice(A$1.us(), A$1.ue())));
  for (; A$1.re(); ) {
    let B = Q$1(E.slice(A$1.es(), A$1.ee()));
    void 0 === B || y.has(B) || o.add(B);
  }
  return { exports: [...o], reexports: [...R] };
}
function Q$1(A) {
  if ('"' !== A[0] && "'" !== A[0]) return A;
  try {
    const B = (function (A) {
      ...
      else if ("'" === B && A.length > 1 && "'" === A[A.length - 1] && -1 === A.indexOf('"'))
        try {
          return JSON.parse('"' + A.slice(1, -1) + '"');
        } catch {}
      let Q = "",
        g = { v: 1 };
      for (; g.v < A.length; ) {
    ...
    for (let A = 0; A < B.length; A++) {
      const Q = 64512 & B.charCodeAt(A);
      if (!(Q < 55296)) {
        if (55296 !== Q) return;
        if (56320 != (64512 & B.charCodeAt(++A))) return;
      }
    }
    return B;
  } catch {}
...
function E$1(A, B) {
  if (B.v === A.length) throw new SyntaxError();
  const Q = A[B.v];
  switch ((++B.v, Q)) {
    case "\r":
      "\n" === A[B.v] && ++B.v;
    ...
    case "x":
      return (function (A, B) {
        ...
        ++B.v;
        const E = g(A[B.v]);
        return (++B.v, String.fromCodePoint(16 * Q + E));
      })(A, B);
    case "u":
      return (function (A, B) {
        let Q = 0;
        if ("{" === A[B.v]) {
          ++B.v;
          do {
            if (((Q = 16 * Q + g(A[B.v])), Q > 1114111)) throw new SyntaxError();
            ++B.v;
          } while ("}" !== A[B.v]);
          ++B.v;
        } else for (let E = 0; E < 4; ++E) ((Q = 16 * Q + g(A[B.v])), ++B.v);
...
function g(A) {
  if (A >= "0" && A <= "9") return +A;
  if (A >= "a" && A <= "f") return A.charCodeAt(0) - 87;
  if (A >= "A" && A <= "F") return A.charCodeAt(0) - 55;
  throw new SyntaxError();
}
function I(A, B) {
  const Q = A.length;
...
function init$1() {
  return (
    D ||
    (D = (async () => {
      const B = await WebAssembly.compile(w()),
        { exports: Q } = await WebAssembly.instantiate(B);
      A$1 = Q;
    })())
  );
}
...
function parse(E, g = "@") {
  ...
  const I = E.length + 1,
    o = (C.__heap_base.value || C.__heap_base) + 4 * I - C.memory.buffer.byteLength;
  ...
  for (; C.ri(); ) {
    const A = C.is(),
      ...
      D = C.se();
    let K;
    C.ip() && (K = k(E.slice(-1 === I ? A - 1 : A, -1 === I ? Q + 1 : Q)));
    const N = [];
    for (C.rsa(); C.ra(); ) {
      const A = C.aks(),
        Q = C.ake(),
        B = C.avs(),
        g = C.ave();
      N.push([J(E.slice(A, Q)), J(E.slice(B, g))]);
    }
    w.push({ n: K, t: B, s: A, e: Q, ss: o, se: D, d: I, a: g, at: N.length > 0 ? N : null });
  }
  for (; C.re(); ) {
    const A = C.es(),
      Q = C.ee(),
      ...
      g = C.ele(),
      I = J(E.slice(A, Q)),
      o = B < 0 ? void 0 : J(E.slice(B, g));
    K.push({ s: A, e: Q, ls: B, le: g, n: I, ln: o });
  }
  function k(A) {
    try {
      return (0, eval)(A);
