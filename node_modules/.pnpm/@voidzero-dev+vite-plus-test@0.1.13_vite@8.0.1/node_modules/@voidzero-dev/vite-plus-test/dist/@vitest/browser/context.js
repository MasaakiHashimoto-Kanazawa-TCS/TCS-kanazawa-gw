    ...
    async upload(element, files) {
      const uploadPromise = (Array.isArray(files) ? files : [files]).map(async (file) => {
        ...
        const {
          ...
          basename,
          mime,
        } = await triggerCommand("__vitest_fileInfo", [file, "base64"]);
...
const page = {
  ...
  async screenshot(options = {}) {
    ...
    const [element, ...mask] = await Promise.all([
      ...
      ...("mask" in options ? options.mask.map((el) => convertToSelector(el, options)) : []),
    ]);
  ...
  mark(name, bodyOrOptions, options) {
    ...
    const hasActiveTrace =
      !!currentTest && getBrowserState().activeTraceTaskIds.has(currentTest.id);
...
function prettyDOM(
  dom,
  maxLength = Number(defaultOptions?.maxLength ?? import.meta.env.DEBUG_PRINT_LIMIT ?? 7e3),
  prettyFormatOptions = {},
) {
  if (maxLength === 0) {
    return "";
...
function getElementError(selector, container) {
  const error = new Error(
    `Cannot find element with locator: ${__INTERNAL._asLocator("javascript", selector)}\n\n${prettyDOM(container)}`,
  );
