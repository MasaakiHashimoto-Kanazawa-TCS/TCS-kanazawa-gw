...
const upload = async (context, selector, files, _options) => {
  ...
  for (const file of files) {
    if (typeof file !== "string") {
      throw new TypeError(
        `The "${context.provider.name}" provider doesn't support uploading files objects. Provide a file path instead.`,
      );
    }
  }
...
class WebdriverBrowserProvider {
    ...
    if (inspector.enabled && (browser === "chrome" || browser === "edge")) {
      ...
      if (host !== "localhost" && host !== "127.0.0.1" && host !== "::1") {
        this.project.vitest.logger.warn(
          `Custom inspector host "${host}" will be ignored. Chrome only allows remote debugging on localhost.`,
        );
      }
  ...
  async _throwIfClosing(action) {
    if (this.closing) {
      debug?.(
        `[%s] provider was closed, cannot perform the action${action ? ` ${action}` : ""}`,
        this.browserName,
      );
      ...
      send: (method, params) => {
        ...
        return this.browser.sendCommandAndGetResult(method, params ?? {}).catch((error) => {
          return Promise.reject(
            new Error(`Failed to execute "${method}" command.`, { cause: error }),
          );
        });
      },
      on: () => {
