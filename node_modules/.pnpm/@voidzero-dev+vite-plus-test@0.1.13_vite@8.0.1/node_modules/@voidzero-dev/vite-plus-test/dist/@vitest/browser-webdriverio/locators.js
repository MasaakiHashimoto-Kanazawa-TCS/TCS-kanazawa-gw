...
page.extend({
  ...
  getByTestId(testId) {
    return new WebdriverIOLocator(
      getByTestIdSelector(window.__vitest_worker__.config.browser.locators.testIdAttribute, testId),
    );
  },
...
function getWebdriverioSelectOptions(element, value) {
  ...
  if (typeof optionValue !== "string") {
    ...
    if (index === -1) {
      throw new Error(
        `The element ${selectorEngine.previewNode(element)} was not found in the "select" options.`,
      );
    }
    return [{ index }];
  }
