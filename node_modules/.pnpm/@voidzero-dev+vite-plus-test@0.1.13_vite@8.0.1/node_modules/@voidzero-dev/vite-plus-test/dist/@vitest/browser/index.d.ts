...
type Comparator<Options extends Record<string, unknown>> = (
  reference: {
...
type CustomComparatorsToRegister = {
  [Key in keyof NonStandardScreenshotComparators]: Comparator<
    NonStandardScreenshotComparators[Key]
  >;
};
...
declare function resolveScreenshotPath(
  testPath: string,
  name: string,
...
declare function defineBrowserCommand<T extends unknown[]>(
  fn: BrowserCommand<T>,
): BrowserCommand<T>;

declare const createBrowserServer: BrowserServerFactory;
declare function defineBrowserProvider<T extends object = object>(
  options: Omit<BrowserProviderOption<T>, "serverFactory" | "options"> & {
    options?: T;
  },
): BrowserProviderOption;
