import {
  ...
  a as getConfig,
  r as relative,
  b as generateFileHash,
} from "./utils-C2ISqq1C.js";
import { channel, globalChannel, client } from "../../@vitest/browser/client.js";
...
class IframeOrchestrator {
  ...
  async runIsolatedTestInIframe(container, spec, options, startTime, otelContext) {
    ...
    const iframe = await this.prepareIframe(container, file, startTime, otelContext);
    await setIframeViewport(iframe, width, height);
    await this.sendEventToIframe({
      event: "execute",
      files: [spec],
...
async function setIframeViewport(iframe, width, height) {
  ...
  if (ui) {
    ...
    await client.rpc.triggerCommand(getBrowserState().sessionId, "__vitest_viewport", void 0, [
      { width, height },
    ]);
  } else {
