...
async function loadNativeEnvironment(name, root, traces) {
  ...
  return resolveEnvironmentFromModule(
    name,
    packageId,
    await traces.$("vitest.runtime.environment.import", () => import(packageId)),
  );
}
    ...
    name[0] === "." || name[0] === "/"
      ...
      : ((
          await traces.$("vitest.runtime.environment.resolve", () =>
            rpc.resolve(`vitest-environment-${name}`, void 0, "__vitest__"),
          )
        )?.id ?? resolve(root, name));
  return {
    environment: resolveEnvironmentFromModule(
      name,
      packageId,
      await traces.$("vitest.runtime.environment.import", () => loader.import(packageId)),
    ),
    loader,
  };
}
        ...
        if (isRunning) {
          send({
            ...
            error: serializeError(
              /* @__PURE__ */ new Error("[vitest-worker]: Worker is already running tests"),
            ),
          });
          return;
        }
