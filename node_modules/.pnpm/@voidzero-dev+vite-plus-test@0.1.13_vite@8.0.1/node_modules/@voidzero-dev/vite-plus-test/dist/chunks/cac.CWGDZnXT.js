  ...
  l$1 = class l extends c$1 {
    ...
    stripOptions(e) {
      ...
      for (; n < e.length; ) {
        ...
        if (r.startsWith(`-`)) {
          n++;
          let t = false,
            i = this.findOption(this, r);
          if (i) t = i.isBoolean ?? false;
          else
    ...
    handleFlagCompletion(e, t, n, r) {
      ...
      if (i) {
        ...
        if (t?.handler) {
          let n = [];
          (t.handler.call(t, (e, t) => n.push({ value: e, description: t }), e.options),
            (this.completions = n));
        }
        return;
      }
      if (n.startsWith(`-`)) {
        let t = n.startsWith(`-`) && !n.startsWith(`--`),
          r = n.replace(/^-+/, ``);
        for (let [i, a] of e.options)
          t && a.alias && `-${a.alias}`.startsWith(n)
            ? this.completions.push({ value: `-${a.alias}`, description: a.description })
            : !t &&
              i.startsWith(r) &&
              this.completions.push({ value: `--${i}`, description: a.description });
      }
    }
    findOption(e, t) {
    ...
    handleCommandCompletion(e, t) {
      ...
      for (let [e, r] of this.commands) {
        if (e === ``) continue;
        let i = e.split(` `);
        i.slice(0, n.length).every((e, t) => e === n[t]) &&
          i[n.length]?.startsWith(t) &&
          this.completions.push({ value: i[n.length], description: r.description });
      }
    }
    handlePositionalCompletion(e, t) {
      let n = e.value.split(` `).length,
        r = Math.max(0, t.length - n),
        i = Array.from(e.arguments.entries());
      if (i.length > 0) {
        let t;
        if (r < i.length) {
          let [e, n] = i[r];
    ...
    complete(e) {
      ...
      let t = new Set();
      (this.completions
        .filter((e) => (t.has(e.value) ? false : (t.add(e.value), true)))
        .filter((t) => {
          if (e.includes(`=`)) {
            let [, n] = e.split(`=`);
        ...
        .forEach((e) => console.log(`${e.value}\t${e.description ?? ``}`)),
    ...
    parse(e) {
      ...
      if (this.shouldCompleteFlags(a, n)) this.handleFlagCompletion(i, r, n, a);
        ...
        (this.shouldCompleteCommands(n) && this.handleCommandCompletion(r, n),
          i && i.arguments.size > 0 && this.handlePositionalCompletion(i, r));
      }
      this.complete(n);
    }
    setup(a, o, s) {
      switch (
        (assert(
          s === `zsh` || s === `bash` || s === `fish` || s === `powershell`,
          `Unsupported shell`,
        ),
        s)
      ) {
...
function r(t, n) {
  let r = t.replace(/[-:]/g, `_`),
    i = a.ShellCompDirectiveError,
    a$1 = a.ShellCompDirectiveNoSpace,
    o = a.ShellCompDirectiveNoFileComp,
    s = a.ShellCompDirectiveFilterFileExt,
    c = a.ShellCompDirectiveFilterDirs;
...
const o = process.execPath,
  s = process.argv.slice(1),
  c = f(o),
  l = s.map(f),
  u = `${c} ${process.execArgv.map(f).join(` `)} ${l[0]}`,
  d = /<[^>]+>|\[[^\]]+\]/;
function f(e) {
  return e.includes(` `) ? `'${e}'` : e;
}
async function p(c, l) {
  for (let e of [c.globalCommand, ...c.commands]) {
    if (e.name === `complete`) continue;
    let t = (e.rawName.match(/\[.*?\]|<.*?>/g) || []).map((e) => e.startsWith(`[`)),
      n = e.name === `@@global@@`,
      r = n ? l : l?.subCommands?.[e.name],
      a = n ? `` : e.name,
      o = n ? u$1 : u$1.command(a, e.description || ``);
    if (o) {
      let n = (e.rawName.match(/<([^>]+)>|\[\.\.\.([^\]]+)\]/g) || []).map((e) =>
      ...
      t.forEach((e, t) => {
        let i = n[t] || `arg${t}`,
          a = r?.args?.[i];
        a ? o.argument(i, a, e) : o.argument(i, void 0, e);
      });
    }
    for (let t of [...c.globalCommand.options, ...e.options]) {
      let e = t.rawName.match(/^-([a-zA-Z]), --/)?.[1],
        a = t.name,
        s = n ? u$1 : o;
      if (s) {
        let n = r?.options?.[a],
          i = t.required || d.test(t.rawName);
        n
          ? e
            ? s.option(a, t.description || ``, n, e)
            : s.option(a, t.description || ``, n)
          : i
            ? e
              ? s.option(a, t.description || ``, async () => [], e)
              : s.option(a, t.description || ``, async () => [])
            : e
              ? s.option(a, t.description || ``, e)
              : s.option(a, t.description || ``);
      }
    }
  }
  return (
    c.command(`complete [shell]`).action(async (l, d) => {
      switch (l) {
        case `zsh`: {
          let t$1 = t(c.name, u);
          console.log(t$1);
        ...
        default: {
          ...
          let e$1 = d[`--`] || [];
          return (
            (c.showHelpOnExit = false),
            c.unsetMatchedCommand(),
            c.parse([o, s[0], ...e$1], { run: false }),
            u$1.parse(e$1)
          );
        }
      }
...
function addCliOptions(cli, options) {
  for (const [optionName, option] of Object.entries(options))
    if (option) addCommand(cli, optionName, option);
}
function createCLI(options = {}) {
  const cli = cac("vp test");
  cli.version(version);
  ...
  cli.help((info) => {
    ...
    const banner = info.find((current) => /^vp test\/[\d.]+$/.test(current.body));
...
async function start(mode, cliFilters, options) {
  try {
    const { startVitest } = await import("./cli-api.DuT9iuvY.js").then(function (n) {
      return n.q;
    });
    ...
    const { errorBanner } = await import("./index.DXMFO5MJ.js").then(function (n) {
      return n.C;
    });
...
async function collect(mode, cliFilters, options) {
  try {
    const { prepareVitest, processCollected, outputFileList } =
      await import("./cli-api.DuT9iuvY.js").then(function (n) {
        return n.q;
      });
    ...
    if (!options.filesOnly) {
      ...
      outputFileList(await ctx.getRelevantTestSpecifications(cliFilters.map(normalize)), options);
    await ctx.close();
  } catch (e) {
    const { errorBanner } = await import("./index.DXMFO5MJ.js").then(function (n) {
      return n.C;
