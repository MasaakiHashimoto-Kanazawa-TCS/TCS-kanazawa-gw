...
} from "../@vitest/mocker/transforms.js";
...
import {
  B as BareModuleMocker,
  n as normalizeModuleId,
...
class NativeModuleMocker extends BareModuleMocker {
  ...
  loadAutomock(url, result) {
    const moduleId = cleanUrl(
      normalizeModuleId(url.startsWith("file://") ? fileURLToPath(url) : url),
    );
    let source;
...
`;
  ...
  loadManualMock(url, result) {
    ...
    try {
      return {
        format: "module",
        source: createManualModuleSource(
          moduleId,
          collectModuleExports(moduleId, transformedCode, format),
        ),
        shortCircuit: true,
      };
    } catch (cause) {
