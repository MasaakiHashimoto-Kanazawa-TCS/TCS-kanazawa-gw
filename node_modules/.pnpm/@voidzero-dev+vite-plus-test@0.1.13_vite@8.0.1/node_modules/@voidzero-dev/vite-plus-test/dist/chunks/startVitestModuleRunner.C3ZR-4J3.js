...
class VitestMocker extends BareModuleMocker {
  ...
  async callFunctionMock(id, url, mock) {
    ...
    const moduleExports = new Proxy(exports$1, {
      get: (target, prop) => {
...
`,
  ...
  async importActual(rawId, importer, callstack) {
    ...
    return await this.moduleRunner.cachedRequest(
      node.url,
      node,
      callstack || [importer],
      void 0,
...
class VitestTransport {
  ...
  async invoke(event) {
    if (event.type !== "custom")
      return {
        error: /* @__PURE__ */ new Error(`Vitest Module Runner doesn't support Vite HMR events.`),
      };
    if (event.event !== "vite:invoke")
      return {
        error: /* @__PURE__ */ new Error(
          `Vitest Module Runner doesn't support ${event.event} event.`,
        ),
      };
    ...
    if (name !== "fetchModule")
      return {
        error: /* @__PURE__ */ new Error(`Unknown method: ${name}. Expected "fetchModule".`),
      };
    try {
...
class VitestModuleRunner extends viteModuleRunner.ModuleRunner {
  ...
  async import(rawId) {
    const resolved = await this._otel.$(
      ...
      { attributes: { "vitest.module.raw_id": rawId } },
      async (span) => {
...
function getCachedVitestImport(id, state) {
  ...
  const relativeRoot =
    relativeIds[root] ?? (relativeIds[root] = normalizedDistDir.slice(root.length));
        ...
        if (isWindows) {
          if (id[1] === ":") {
            ...
            if (id[0] !== cwd[0] && id[0].toUpperCase() === cwd[0].toUpperCase())
              id =
                (cwd[0].toUpperCase() === cwd[0] ? id[0].toUpperCase() : id[0].toLowerCase()) +
                id.slice(1);
...
export {
  BareModuleMocker as B,
  VITEST_VM_CONTEXT_SYMBOL as V,
  VitestModuleRunner as a,
  VitestTransport as b,
  createNodeImportMeta as c,
  normalizeModuleId as n,
  startVitestModuleRunner as s,
};
