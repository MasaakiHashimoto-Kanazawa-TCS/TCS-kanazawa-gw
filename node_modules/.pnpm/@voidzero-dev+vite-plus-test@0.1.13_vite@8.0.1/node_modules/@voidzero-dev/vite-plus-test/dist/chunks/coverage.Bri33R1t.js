...
import { slash, shuffle, toArray, cleanUrl } from "../@vitest/utils/helpers.js";
import { resolve, relative, normalize } from "../vendor/pathe.mjs";
import pm from "../vendor/picomatch.mjs";
import { glob } from "../vendor/tinyglobby.mjs";
import c from "../vendor/tinyrainbow.mjs";
import {
  c as configDefaults,
  e as benchmarkConfigDefaults,
  a as coverageConfigDefaults,
} from "./defaults.CdU2lD-q.js";
import crypto from "node:crypto";
import { r as resolveModule } from "./index.BCY_7LL2.js";
import { mergeConfig } from "@voidzero-dev/vite-plus-core";
import {
  c as configFiles,
  d as defaultBrowserPort,
  a as defaultInspectPort,
  b as defaultPort,
} from "./constants.CPYnjOGj.js";
import "./env.D4Lgay0q.js";
import nodeos__default from "node:os";
import { isCI, isAgent, provider } from "../vendor/std-env.mjs";
...
class BaseSequencer {
  ...
  async shard(files) {
    ...
    return [...files]
      .map((spec) => {
        const specPath = resolve(slash(config.root), slash(spec.moduleId))?.slice(
          config.root.length,
        );
        return {
          spec,
...
function resolveConfig$1(vitest, options, viteConfig) {
  ...
  if (
    ...
    logger.console.warn(
      c.yellow(
        "Warning: retry.condition function cannot be used inside a config file. Use a RegExp pattern instead, or define the function in your test file.",
      ),
    );
    resolved.retry = {
      ...resolved.retry,
  ...
  if ("poolOptions" in resolved)
    logger.deprecate(
      "`test.poolOptions` was removed in Vitest 4. All previous `poolOptions` are now top-level options. Please, refer to the migration guide: https://vitest.dev/guide/migration#pool-rework",
    );
  resolved.pool ??= "forks";
    ...
    const browserConfig = `
{
  browser: {
    ...
    const preferredProvider =
      !browser.provider?.name || browser.provider.name === "preview"
        ? "playwright"
      ...
      const coverageExample = `
{
  ...
  if (resolved.coverage.changed === void 0 && resolved.changed !== void 0)
  ...
  if (resolved.coverage.enabled && resolved.coverage.reportsDirectory) {
    ...
    if (!resolved.coverage.htmlDir) {
      ...
      } else if (resolved.coverage.reporter.find(([name]) => name === "lcov"))
  ...
  if (mode === "benchmark") {
    ...
    const reporters = Array.from(
      new Set([...toArray(resolved.benchmark.reporters), ...toArray(options.reporter)]),
    ).filter(Boolean);
      ...
      for (const reporter of resolved.reporters)
        if (Array.isArray(reporter)) {
          ...
          if (typeof reporterName === "string")
            configReportersMap.set(reporterName, reporterOptions);
        }
  ...
  if (!isPreview && resolved.browser.enabled && provider === "stackblitz")
    throw new Error(
  ...
  if (
    toArray(resolved.reporters).some((reporter) => {
...
function hasBrowserChromium(vitest, config) {
  ...
  return browser.instances.some((instance) => {
    const name =
      instance.name || (config.name ? `${config.name} (${instance.browser})` : instance.browser);
...
class BaseCoverageProvider {
  ...
  _initialize(ctx) {
    ...
    if (ctx.version !== this.version)
      ctx.logger.warn(
        c.yellow(`Loaded ${c.inverse(c.yellow(` vitest@${ctx.version} `))} and ${c.inverse(c.yellow(` @vitest/coverage-${this.name}@${this.version} `))}.
...
Update your dependencies and make sure the versions match.`),
  ...
  async onTestRunStart() {
    if (this.options.changed) {
      ...
      this.changedFiles =
        (await new VitestGit(this.ctx.config.root).findChangedFiles({
  ...
  checkThresholds(allThresholds) {
    for (const { coverageMap, thresholds, name } of allThresholds) {
      if (
        thresholds.branches === void 0 &&
        thresholds.functions === void 0 &&
        thresholds.lines === void 0 &&
        thresholds.statements === void 0
      )
        continue;
  ...
  async updateThresholds({ thresholds: allThresholds, onUpdate, configurationFile }) {
    ...
    for (const { coverageMap, thresholds, name } of allThresholds) {
      const summaries = this.options.thresholds?.perFile
        ? coverageMap.files().map((file) => coverageMap.fileCoverageFor(file).toSummary())
  ...
  async transformFile(url, project, viteEnvironment) {
    ...
    if (project.isBrowserEnabled() || viteEnvironment === "__browser__") {
      const result = await (
        project.browser?.vite.environments.client || project.vite.environments.client
      ...
      for (const project of projects) {
        ...
        try {
          ...
          const viteEnvironment =
            environment === "jsdom" || environment === "happy-dom" ? "client" : "ssr";
...
function resolveGlobThresholds(thresholds) {
  ...
  return {
    lines:
      "lines" in thresholds && typeof thresholds.lines === "number" ? thresholds.lines : void 0,
    branches:
...
export {
  BaseCoverageProvider as B,
  RandomSequencer as R,
  BaseSequencer as a,
  resolveApiServerConfig as b,
  getCoverageProvider as g,
  hash as h,
  isBrowserEnabled as i,
  resolveConfig$1 as r,
};
