...
}[keyof T]; // A union of the properties of the given array T that can be used to index it.
...
type IndexValue<T, P> = T extends any[]
  ...
  : P extends keyof T
    ? T[P]
    : never; // Determines if an object T is an array like string[] (in which case this
...
declare class AstPath<T = any> {
  ...
  ): boolean; // For each of the tree walk functions (call, each, and map) this provides 5
...
interface Printer<T = any> {
  ...
  embed?:
    | ((
        ...
        | ((
            textToDoc: (text: string, options: Options$1) => Promise<Doc$1>,
            print: (selector?: string | number | Array<string | number> | AstPath) => Doc$1,
            path: AstPath,
            options: Options$1,
          ) => Promise<Doc$1 | undefined> | Doc$1 | undefined)
        | Doc$1
        | null)
    | undefined;
