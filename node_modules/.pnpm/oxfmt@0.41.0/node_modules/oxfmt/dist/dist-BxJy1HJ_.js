...
import {
  ...
  r as maybeResolve,
  t as expiringMap,
} from "./resolve-pWjAK-4f-C1Z6M6r2.js";
import {
  i as visit,
  n as cacheForDirs,
  r as spliceChangesIntoString,
} from "./utils-D8dQkKEd-BtAa7w-M.js";
import {
  a as sortClasses,
  i as sortClassList,
  n as error,
  o as warn,
  r as getTailwindConfig$1,
} from "./sorter-BZkvDMjt-tVbe6MxP.js";
...
var import_line_column = /* @__PURE__ */ __toESM(
  /* @__PURE__ */ __commonJSMin((exports, module) => {
    ...
    LineColumnFinder.prototype.toIndex = function (line, column) {
      ...
      if (line >= 0 && column >= 0 && line < this.lineToIndex.length) {
        ...
        var nextIndex =
          line === this.lineToIndex.length - 1 ? this.str.length : this.lineToIndex[line + 1];
  ...
  for (let opts of transforms) {
    for (let [name, meta] of Object.entries(opts.parsers))
      parsers[name] = async () => {
        var _plugin$parsers;
        let original =
          (_plugin$parsers = (await loadPlugins(meta.load ?? opts.load ?? [])).parsers) === null ||
          _plugin$parsers === void 0
            ? void 0
            : _plugin$parsers[name];
        if (!original) return;
    ...
    for (let [name, _meta] of Object.entries(opts.printers ?? {}))
      printers[name] = async () => {
        ...
        let original =
          (_plugin$printers = (await loadPlugins(opts.load ?? [])).printers) === null ||
          _plugin$printers === void 0
            ? void 0
            : _plugin$printers[name];
        if (!original) return;
  ...
  async function load(options) {
    ...
    for (const pluginName of opts.compatible || []) {
      ...
      if (
        plugin === null ||
        plugin === void 0 ||
        (_plugin$parsers2 = plugin.parsers) === null ||
        _plugin$parsers2 === void 0
          ? void 0
          : _plugin$parsers2[name]
      )
...
async function loadTailwindCSS({ options, opts }) {
  ...
  return {
    ...
    matcher: createMatcher(options, parser, {
      staticAttrs: new Set(
        ((_parsers$parser = parsers[parser]) === null || _parsers$parser === void 0
          ? void 0
          : _parsers$parser.staticAttrs) ??
          opts.staticAttrs ??
          [],
      ),
      dynamicAttrs: new Set(
        ((_parsers$parser2 = parsers[parser]) === null || _parsers$parser2 === void 0
          ? void 0
          : _parsers$parser2.dynamicAttrs) ??
          opts.dynamicAttrs ??
          [],
      ),
...
function transformDynamicJsAttribute(attr, env) {
  ...
  function findConcatEntry(path) {
    return path.find((entry) => {
      var _entry$parent;
      return (
        ((_entry$parent = entry.parent) === null || _entry$parent === void 0
          ? void 0
          : _entry$parent.type) === "BinaryExpression" && entry.parent.operator === "+"
      );
    });
  }
...
function transformHtml(ast, env) {
  ...
  for (let attr of ast.attrs ?? [])
    if (matcher.hasStaticAttr(attr.name)) attr.value = sortClasses(attr.value, { env });
...
function sortStringLiteral(
  ...
  node.value = result;
  let raw =
    ((_node$extra3 = node.extra) === null || _node$extra3 === void 0 ? void 0 : _node$extra3.raw) ??
    node.raw;
  let quote = raw[0];
  let originalRawContent = raw.slice(1, -1);
  let originalValue =
    ((_node$extra4 = node.extra) === null || _node$extra4 === void 0
      ? void 0
      : _node$extra4.rawValue) ?? node.value;
  if (node.extra) {
    ...
    if (entry.parent.type === "TemplateLiteral") {
      ...
      for (let quasi of entry.parent.quasis) {
        ...
        if (nodeStart !== null && quasiEnd !== null && nodeStart - quasiEnd <= 2)
          start && (start = /^\s/.test(quasi.value.raw));
        if (nodeEnd !== null && quasiStart !== null && nodeEnd - quasiStart <= 2)
          end && (end = /\s$/.test(quasi.value.raw));
      }
    }
  }
  return {
        ...
        switch (currentNode.value.type) {
          case "ArrayExpression":
            ...
            for (const node of classList)
              if (node.type === "StringLiteral") node.value = sortClasses(node.value, { env });
            break;
          case "StringLiteral":
  ...
  visit(ast, {
    ...
    StringLiteral(node, path, meta) {
      ...
      node.value = sortClasses(node.value, {
        env,
        ignoreFirst:
          (concat === null || concat === void 0 ? void 0 : concat.key) === "right" &&
          !/^[^\S\r\n]/.test(node.value),
        ignoreLast:
          (concat === null || concat === void 0 ? void 0 : concat.key) === "left" &&
          !/[^\S\r\n]$/.test(node.value),
        collapseWhitespace: {
          start: (concat === null || concat === void 0 ? void 0 : concat.key) !== "right",
          end: (concat === null || concat === void 0 ? void 0 : concat.key) !== "left",
...
function transformSvelte(ast, env) {
  ...
  if (ast.type === "IfBlock") {
    var _ast$else;
    for (let child of ((_ast$else = ast.else) === null || _ast$else === void 0
      ? void 0
      : _ast$else.children) ?? [])
