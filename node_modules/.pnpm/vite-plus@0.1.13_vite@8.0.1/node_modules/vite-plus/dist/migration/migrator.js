...
export function checkViteVersion(projectPath) {
  return checkPackageVersion(projectPath, "vite", "7.0.0");
}
export function checkVitestVersion(projectPath) {
...
export function detectEslintProject(projectPath, packages) {
...
export function detectPrettierProject(projectPath, packages) {
  const packageJsonPath = path.join(projectPath, "package.json");
  if (!fs.existsSync(packageJsonPath)) {
    return { hasDependency: false };
  }
 ...
 */
export function rewriteMonorepo(workspaceInfo, skipStagedMigration, silent = false, report) {
...
export function rewritePackageJson(pkg, packageManager, isMonorepo, skipStagedMigration) {
  if (pkg.scripts) {
 ...
 */
export function mergeViteConfigFiles(projectPath, silent = false, report) {
  const configs = detectConfigs(projectPath);
 ...
 */
export function injectLintTypeCheckDefaults(projectPath, silent = false, report) {
  if (hasBaseUrlInTsconfig(projectPath)) {
    return;
  }
 ...
 */
export function mergeStagedConfigToViteConfig(projectPath, stagedConfig, silent = false, report) {
  const configs = detectConfigs(projectPath);
  const viteConfig = ensureViteConfig(projectPath, configs, silent, report);
  const fullViteConfigPath = path.join(projectPath, viteConfig);
 ...
 */
export function hasStagedConfigInViteConfig(projectPath) {
  const configs = detectConfigs(projectPath);
  if (!configs.viteConfig) {
    return false;
  }
 ...
 */
export function installGitHooks(projectPath, silent = false, report) {
 ...
 */
export function getOldHooksDir(rootDir) {
 ...
 */
export function preflightGitHooksSetup(projectPath) {
 ...
 */
export function setupGitHooks(projectPath, oldHooksDir, silent = false, report) {
 ...
 */
export function ensurePreCommitHook(projectPath, dir = ".vite-hooks") {
  if (!hasStagedConfigInViteConfig(projectPath)) {
    mergeStagedConfigToViteConfig(projectPath, DEFAULT_STAGED_CONFIG, true);
  }
  createPreCommitHook(projectPath, dir);
}
export function createPreCommitHook(projectPath, dir = ".vite-hooks") {
 ...
 */
export function rewritePrepareScript(rootDir) {
  const packageJsonPath = path.join(rootDir, "package.json");
  if (!fs.existsSync(packageJsonPath)) {
    return;
  }
