...
import {
  ...
  o as success,
  t as renderCliDoc,
} from "./help-HviKaKAU.js";
import {
  a as detectExistingEditor,
  n as updatePackageJsonWithDeps,
  o as selectEditor,
  r as updateWorkspaceConfig,
...
function prependToPathToEnvs(extraPath, envs) {
  ...
  if (!current.split(delimiter).filter(Boolean).includes(extraPath))
...
function discoverTemplate(templateName, templateArgs, workspaceInfo, interactive) {
  ...
  if (localPackage) {
    ...
    let binPath = "";
    if (pkg.bin)
      if (typeof pkg.bin === "string") binPath = path.join(localPackagePath, pkg.bin);
      else {
...
async function runRemoteTemplateCommand(
  workspaceInfo,
  cwd,
  templateInfo,
  detectCreatedProjectDir,
  silent = false,
) {
...
async function executeBuiltinTemplate(workspaceInfo, templateInfo, options) {
  ...
  if (templateInfo.command === BuiltinTemplate.application) {
    ...
    setPackageName(
      path.join(workspaceInfo.rootDir, templateInfo.targetDir),
...
async function executeMonorepoTemplate(workspaceInfo, templateInfo, interactive, options) {
  ...
  const libraryResult = await runRemoteTemplateCommand(
    workspaceInfo,
    fullPath,
    discoverTemplate(LibraryTemplateRepo, [libraryDir], workspaceInfo),
    false,
    options?.silent ?? false,
  );
...
function showCreateSummary(options) {
  ...
  log$1(
    `${styleText("magenta", "◇")} Scaffolded ${accent(projectDir)}${description ? ` with ${description}` : ""}`,
  );
  log$1(
  ...
  if (installSummary?.status === "installed")
    log$1(
      `${styleText("green", "✓")} Dependencies installed in ${formatDuration(installSummary.durationMs)}`,
    );
...
`);
  ...
  if (isMonorepo && options.interactive && !targetDir) {
    ...
    if (workspaceInfoOptional.parentDirs.length > 0 || isInSubdirectory) {
      ...
      const selected = await select({
        ...
        options: dirOptions,
        initialValue: shouldOfferCwdOption
          ? cwdRelativeToRoot
  ...
  const shouldSilencePackageManagerInstallLog =
    compactOutput || (isMonorepo && workspaceInfoOptional.packageManager !== void 0);
  ...
  const createProgress =
    options.interactive && compactOutput ? spinner({ indicator: "timer" }) : void 0;
  ...
  if (templateInfo.command === BuiltinTemplate.monorepo) {
    ...
    const installSummary = await runViteInstall(fullPath, options.interactive, void 0, {
      silent: compactOutput,
    });
  ...
  if (isMonorepo) {
    ...
    updateCreateProgress("Installing dependencies");
    installSummary = await runViteInstall(workspaceInfo.rootDir, options.interactive, void 0, {
      silent: compactOutput,
    });
    updateCreateProgress("Formatting code");
    ...
    if (shouldSetupHooks) installGitHooks(fullPath, compactOutput);
    updateCreateProgress("Installing dependencies");
    installSummary = await runViteInstall(fullPath, options.interactive, void 0, {
      silent: compactOutput,
    });
    updateCreateProgress("Formatting code");
    await runViteFmt(fullPath, options.interactive, void 0, { silent: compactOutput });
