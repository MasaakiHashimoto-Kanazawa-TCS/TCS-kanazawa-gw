import { n as eastAsianWidth, t as stripAnsi } from "./strip-ansi-DbamwI77.js";
import { t as sliceAnsi } from "./slice-ansi-1i6Zdm-R.js";
...
function cliTruncate(text, columns, options = {}) {
  ...
  function leadingSgrSpanEndIndex(string) {
    ...
    while (
      ...
      string.codePointAt(index) === ANSI.ESC &&
      string.codePointAt(index + 1) === ANSI.LEFT_BRACKET
    ) {
      let j = index + 2;
  ...
  function trailingSgrSpanStartIndex(string) {
    ...
    while (start > 1 && string.codePointAt(start - 1) === ANSI.LETTER_M) {
      ...
      if (
        j >= 1 &&
        string.codePointAt(j - 1) === ANSI.ESC &&
        string.codePointAt(j) === ANSI.LEFT_BRACKET
      ) {
        start = j - 1;
        continue;
      }
  ...
  if (position === "end") {
    if (preferTruncationOnSpace)
      return appendWithInheritedStyleFromEnd(
        sliceAnsi(text, 0, getIndexOfNearestSpace(text, columns - 1)),
        truncationCharacter,
      );
    if (space) truncationCharacter = ` ${truncationCharacter}`;
    return appendWithInheritedStyleFromEnd(
      sliceAnsi(text, 0, columns - stringWidth(truncationCharacter)),
      truncationCharacter,
    );
  }
  throw new Error(
