...
import {
  A as displayRelative,
  B as outro,
  C as defaultInteractive,
  D as runViteInstall,
  H as spinner,
  I as confirm,
  N as PackageManager,
  O as selectPackageManager,
  P as require_semver,
  R as log,
  S as cancelAndExit,
  T as promptGitHooks,
  ...
  f as mergeViteConfigFiles,
  g as rewriteMonorepo,
  h as preflightGitHooksSetup,
  k as upgradeYarn,
  l as detectPrettierProject,
  m as migratePrettierToOxfmt,
  n as detectExistingAgentTargetPaths,
  o as checkViteVersion,
  p as migrateEslintToOxlint,
  r as selectAgentTargetPaths,
  s as checkVitestVersion,
  t as detectAgentConflicts,
  v as rewriteStandaloneProject,
  w as downloadPackageManager$1,
  y as createMigrationReport,
...
import {
  i as detectEditorConflicts,
  o as selectEditor,
  s as writeEditorConfigs,
  t as detectWorkspace$1,
} from "./workspace-CVWspacc.js";
...
function warnLegacyEslintConfig(legacyConfigFile) {
  log.warn(
    `Legacy ESLint configuration detected (${legacyConfigFile}). Automatic migration to Oxlint requires ESLint v9+ with flat config format (eslint.config.*). Please upgrade to ESLint v9 first: https://eslint.org/docs/latest/use/migrate-to-9.0.0`,
  );
}
...
async function promptEslintMigration(projectPath, interactive, packages) {
  ...
  if (!(await migrateEslintToOxlint(projectPath, interactive, eslintProject.configFile, packages)))
...
async function promptPrettierMigration(projectPath, interactive, packages) {
  ...
  if (
    !(await migratePrettierToOxfmt(projectPath, interactive, prettierProject.configFile, packages))
  )
...
async function collectMigrationPlan(rootDir, detectedPackageManager, options, packages) {
  ...
  const existingAgentTargetPaths =
    options.agent !== void 0 || !options.interactive
      ? void 0
      : detectExistingAgentTargetPaths(rootDir);
  const selectedAgentTargetPaths =
    existingAgentTargetPaths !== void 0
...
function showMigrationSummary(options) {
  ...
  log$1(
    `${styleText("magenta", "◇")} ${updatedExistingVitePlus ? "Updated" : "Migrated"} ${accent(projectLabel)}${updatedExistingVitePlus ? "" : " to Vite+"}`,
  );
  log$1(
  ...
  if (configUpdates > 0 || report.rewrittenImportFileCount > 0) {
    ...
    if (report.rewrittenImportFileCount > 0)
      parts.push(
        `${report.rewrittenImportFileCount} ${report.rewrittenImportFileCount === 1 ? "file had" : "files had"} imports rewritten`,
      );
...
async function executeMigrationPlan(workspaceInfoOptional, plan, interactive) {
  ...
  const downloadResult = await downloadPackageManager$1(
    plan.packageManager,
  ...
  if (
    ...
    import_semver.default.satisfies(downloadResult.version, ">=4.0.0 <4.10.0")
  ) {
    ...
    import_semver.default.satisfies(downloadResult.version, "< 9.5.0")
  ) {
    ...
    import_semver.default.satisfies(downloadResult.version, "< 8.3.0")
  ) {
    failMigrationProgress("Migration failed");
    log.error(
