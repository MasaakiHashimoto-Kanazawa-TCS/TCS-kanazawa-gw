...
const execGit = async (cmd, options) => {
  ...
  const result = z("git", [...NO_SUBMODULE_RECURSE, ...cmd], {
    nodeOptions: {
      cwd: options?.cwd,
      stdio: ["ignore"],
    },
...
var eventemitter3_default = /* @__PURE__ */ __toESM(
...
var import_rfdc = /* @__PURE__ */ __toESM(
  /* @__PURE__ */ __commonJSMin((exports, module) => {
    module.exports = rfdc;
  ...
  suffix(message, ...suffixes) {
    suffixes.filter(Boolean).forEach((suffix) => {
      ...
      if (typeof suffix === "string") message += this.wrap(suffix);
        ...
        suffix.args ??= [];
        if (
          typeof suffix.condition === "function"
            ? !suffix.condition(...suffix.args)
            : !(suffix.condition ?? true)
        )
          return message;
        message += this.wrap(
          typeof suffix.field === "function" ? suffix.field(...suffix.args) : suffix.field,
  ...
  prefix(message, ...prefixes) {
    prefixes.filter(Boolean).forEach((prefix) => {
      ...
      if (typeof prefix === "string") message = this.wrap(prefix) + message;
        ...
        prefix.args ??= [];
        if (
          typeof prefix.condition === "function"
            ? !prefix.condition(...prefix.args)
            : !(prefix.condition ?? true)
        )
          return message;
        message =
          this.wrap(
            typeof prefix.field === "function" ? prefix.field(...prefix.args) : prefix.field,
  ...
  write(data, ...args) {
    ...
    this.buffer.push({
      ...
      entry: this.decoder.write(
        typeof data === "string"
          ? Buffer.from(data, typeof args[0] === "string" ? args[0] : void 0)
          : Buffer.from(data),
      ),
    });
...
var ProcessOutput = class {
  ...
  release() {
    const output = Object.entries(this.stream)
      ...
      .filter((output$1) => this.options.dump.includes(output$1.name))
      .flatMap((output$1) => output$1.buffer)
...
let ListrDefaultRendererLogLevels = /* @__PURE__ */ (function (ListrDefaultRendererLogLevels$1) {
  ...
  ListrDefaultRendererLogLevels$1["COMPLETED_WITH_FAILED_SUBTASKS"] =
    "COMPLETED_WITH_FAILED_SUBTASKS";
  ListrDefaultRendererLogLevels$1["COMPLETED_WITH_FAILED_SISTER_TASKS"] =
...
var DefaultRenderer = class DefaultRenderer {
  ...
  async render() {
    const { createLogUpdate } = await import("./log-update-Beldo2tV.js");
    const { default: truncate } = await import("./cli-truncate-BmIULUG7.js");
    const { default: wrap } = await import("./wrap-ansi-DAqB-SZm.js");
  ...
  shouldOutputToOutputBar(task) {
    ...
    return (
      (typeof outputBar === "number" && outputBar !== 0) ||
      (typeof outputBar === "boolean" && outputBar !== false)
    );
  }
  ...
  renderer(tasks, level = 0) {
    return tasks.flatMap((task) => {
      ...
      if (task.hasTitle())
        if (
          !(
            tasks.some((task$1) => task$1.hasFailed()) &&
            !task.hasFailed() &&
          ...
          if (task.hasFailed() && rendererOptions.collapseErrors)
            ...
            output.push(
              ...this.format(
                this.logger.suffix(
                  task.message.skip && rendererOptions.showSkipMessage
                    ? task.message.skip
                    : task.title,
                  {
            ...
            task.isCompleted() &&
            task.hasTitle() &&
            assertFunctionOrSelf(rendererTaskOptions.timer?.condition, task.message.duration)
          )
            output.push(
              ...this.format(
                this.logger.suffix(task?.title, {
      ...
      if (
        ...
        (task.isPending() ||
          ...
          (task.isCompleted() &&
            ...
            !task.subtasks.some(
              (subtask) => this.cache.rendererOptions.get(subtask.id)?.collapseSubtasks === true,
            )) ||
          task.subtasks.some(
            (subtask) => this.cache.rendererOptions.get(subtask.id)?.collapseSubtasks === false,
          ) ||
    ...
    if (this.shouldOutputToBottomBar(task) && !this.buffer.bottom.has(task.id)) {
      this.buffer.bottom.set(
        ...
        new ProcessOutputBuffer({
          limit:
            typeof rendererTaskOptions.bottomBar === "number" ? rendererTaskOptions.bottomBar : 1,
        }),
      );
      ...
      this.buffer.output.set(
        ...
        new ProcessOutputBuffer({
          limit:
            typeof rendererTaskOptions.outputBar === "number" ? rendererTaskOptions.outputBar : 1,
        }),
      );
      task.on(ListrTaskEventType.OUTPUT, (output) => {
  ...
  dump(task, level, source = ListrLogLevels.OUTPUT, data) {
    ...
    if (
      (task.hasTitle() && source === ListrLogLevels.FAILED && data === task.title) ||
      typeof data !== "string"
    )
      return [];
  ...
  set state$(state) {
    ...
    if (this.hasSubtasks() && this.hasFailed()) {
      for (const subtask of this.subtasks)
        if (subtask.state === ListrTaskState.STARTED) subtask.state$ = ListrTaskState.FAILED;
    }
...
const incorrectBraces = (before, after) =>
  yellow$1(`${warning} Detected incorrect braces with only single value: \`${before}\`. Reformatted as: \`${after}\`
`);
...
const skippingBackup = (hasInitialCommit, diff) => {
  return yellow$1(
    `${warning} Skipping backup because ${diff !== void 0 ? "`--diff` was used" : (hasInitialCommit ? "`--no-stash` was used" : "there’s no initial commit yet") + ". This might result in data loss"}.\n`,
  );
};
const SKIPPING_HIDE_PARTIALLY_CHANGED = yellow$1(
...
const DEPRECATED_GIT_ADD =
  yellow$1(`${warning} Some of your tasks use \`git add\` command. Please remove it from the config since all modifications made by tasks will be automatically added to the git commit index.
`);
...
const failedToLoadConfig = (filepath) =>
  red$1(`${error} Failed to read config from file "${filepath}".`);
const failedToParseConfig = (
  filepath,
  error,
) => `${red$1(`${error} Failed to parse config from file "${filepath}".`)}
...
const ConfigFormatError = /* @__PURE__ */ new Error(
  "Configuration should be an object or a function",
);
...
const shouldRestoreUnstagedChanges = (ctx) =>
...
const getDiffCommand = (diff, diffFilter) => {
  ...
  return ["diff", `--diff-filter=${diffFilterArg}`, ...diffArgs];
};
//#endregion
...
var GitWorkflow = class {
  ...
  async getBackupStash(ctx) {
    const index = (
      await this.execGit(["stash", "list", '--format="%h %s"', "-z"])
        .then(parseGitZOutput)
        .then(cleanGitStashOutput)
    ).findIndex((line) => line.startsWith(ctx.backupHash));
    if (index === -1) {
  ...
  async getDeletedFiles() {
    ...
    const deletedFiles = (await this.execGit(["ls-files", "--deleted"]))
      .split("\n")
      .filter(Boolean)
  ...
  async restoreMergeStatus(ctx) {
    ...
    try {
      ...
      debugLog$8(error);
      handleError(
        /* @__PURE__ */ new Error("Merge state could not be restored due to an error!"),
        ctx,
  ...
  async getUnstagedFiles({ onlyPartial = false } = {}) {
     ...
     * (e.g. `to`\0`from`)
     */
    const unstagedFiles = (await this.execGit(["status", "-z"]))
      .split(/\x00(?=[ AMDRCU?!]{2} |$)/)
      .filter((line) => {
  ...
  async runTasks(ctx, task, { listrTasks, concurrent }) {
    if (ctx.shouldFailOnChanges) {
      ...
      ctx.unstagedDiffSha256 = calculateSha256(
        await this.execGit(["diff", "--patch", "--unified=0"]),
      );
  ...
  async applyModifications(ctx) {
    if (ctx.shouldFailOnChanges) {
      ...
      const diffSha256 = calculateSha256(await this.execGit(["diff", "--patch", "--unified=0"]));
    ...
    if (
      !(await this.execGit([...getDiffCommand(this.diff, this.diffFilter), "--name-only", "-z"])) &&
      !this.allowEmpty
    )
      handleError(
        /* @__PURE__ */ new Error("Prevented an empty git commit!"),
        ctx,
...
const resolveGitRepo = async (cwd = process.cwd()) => {
  try {
    ...
    const [relativeTopLevelDir, topLevel, absoluteGitDir] = (
      await execGit(["rev-parse", CDUP, TOPLEVEL, ABSOLUTE_GIT_DIR], { cwd })
    ).split("\n");
...
const listConfigFilesFromGit = async ({ cwd, topLevelDir }) =>
  execGit([...EXEC_GIT, "--cached", "--others", "--exclude-standard", "--", ...CONFIG_PATHSPEC], {
    cwd,
  })
    .then(parseGitZOutput)
    .then((lines) => {
...
const runAll = async (
  ...
  for (const [configPath, { config, files }] of Object.entries(filesByConfig)) {
    ...
    for (const [index, files] of stagedFileChunks.entries()) {
      ...
      listrTasks.push({
        title:
          `${configName}${blackBright$1(` — ${files.length} ${files.length > 1 ? "files" : "file"}`)}` +
          (chunkCount > 1 ? blackBright$1(` (chunk ${index + 1}/${chunkCount})...`) : ""),
        task: (ctx, task) =>
          task.newListr(chunkListrTasks, {
            concurrent,
            exitOnError: !continueOnError,
          }),
        skip: () => {
...
async function resolveViteConfig(cwd) {
  const { resolveConfig } = await import("./src-FkMgH-SM.js");
