...
import {
  lstatSync as lstatSync$1,
  readdir as readdir$1,
  readdirSync as readdirSync$1,
  readlinkSync,
  realpathSync,
} from "fs";
...
async function writeEditorConfigs({
  projectRoot,
  editorId,
  interactive,
  conflictDecisions,
  ...
  for (const [fileName, incoming] of Object.entries(editorConfig.files)) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      if (preResolved) conflictAction = preResolved;
        ...
        conflictAction = Ct(action) || action === "skip" ? "skip" : "merge";
...
const parseClass = (glob, position) => {
    ...
    if (c === "[" && !escaping) {
      for (const [cls, [unip, u, neg]] of Object.entries(posixClasses))
        if (glob.startsWith(cls, i)) {
          if (rangeStart) return ["$.", false, glob.length - pos, true];
          i += cls.length;
  ...
  if (!ranges.length && !negs.length) return ["$.", false, glob.length - pos, true];
...
var AST = class {
  ...
  toRegExpSource(allowDot) {
    ...
    if (!isExtglobAST(this)) {
      ...
      return [start + src + end, unescape(src), (this.#hasMagic = !!this.#hasMagic), this.#uflag];
    }
...
var Minimatch = class {
  ...
  hasMagic() {
    ...
    for (const pattern of this.set)
      for (const part of pattern) if (typeof part !== "string") return true;
    return false;
  }
  debug(..._) {}
  make() {
    ...
    if (this.options.noglobstar) {
      for (let i = 0; i < globParts.length; i++)
        for (let j = 0; j < globParts[i].length; j++)
          if (globParts[i][j] === "**") globParts[i][j] = "*";
    }
  ...
  #matchGlobStarBodySections(
    ...
    while (fileIndex <= after) {
      if (
        this.#matchOne(file.slice(0, fileIndex + body.length), body, partial, fileIndex, 0) &&
        globStarDepth < this.maxGlobstarRecursion
      ) {
  ...
  *rvalues() {
      ...
      if (v !== oldVal) {
        if (this.#hasFetchMethod && this.#isBackgroundFetch(oldVal)) {
          ...
          if (s !== void 0 && !noDisposeOnSet) {
            ...
            if (this.#hasDisposeAfter) this.#disposed?.push([s, k, "set"]);
          }
        } else if (!noDisposeOnSet) {
          if (this.#hasDispose) this.#dispose?.(oldVal, k, "set");
          if (this.#hasDisposeAfter) this.#disposed?.push([oldVal, k, "set"]);
        }
        this.#removeItemSize(index);
        this.#addItemSize(index, size, status);
        this.#valList[index] = v;
  ...
  #evict(free) {
    ...
    if (this.#hasFetchMethod && this.#isBackgroundFetch(v))
      v.__abortController.abort(/* @__PURE__ */ new Error("evicted"));
    else if (this.#hasDispose || this.#hasDisposeAfter) {
      if (this.#hasDispose) this.#dispose?.(v, k, "evict");
      if (this.#hasDisposeAfter) this.#disposed?.push([v, k, "evict"]);
    }
  ...
  #backgroundFetch(k, index, options, context) {
    ...
    const cb = (v, updateCache = false) => {
      ...
      if (vl === p || (ignoreAbort && updateCache && vl === void 0))
        if (v === void 0)
          if (bf.__staleWhileFetching !== void 0) this.#valList[index] = bf.__staleWhileFetching;
    ...
    const fetchFail = (er) => {
      ...
      if (allowStale) {
        if (options.status && bf.__staleWhileFetching !== void 0)
    ...
    if (this.#size !== 0) {
      ...
      if (index !== void 0) {
        ...
        if (this.#size === 1) this.#clear(reason);
          ...
          const v = this.#valList[index];
          if (this.#isBackgroundFetch(v))
            v.__abortController.abort(/* @__PURE__ */ new Error("deleted"));
          else if (this.#hasDispose || this.#hasDisposeAfter) {
            if (this.#hasDispose) this.#dispose?.(v, k, reason);
            if (this.#hasDisposeAfter) this.#disposed?.push([v, k, reason]);
          }
          this.#keyMap.delete(k);
...
const isStream = (s) =>
  !!s &&
  typeof s === "object" &&
  (s instanceof Minipass || s instanceof Stream || isReadable(s) || isWritable(s));
/**
...
var Minipass = class extends EventEmitter {
  ...
  write(chunk, encoding, cb) {
    ...
    if (this[DESTROYED]) {
      this.emit(
        "error",
        Object.assign(/* @__PURE__ */ new Error("Cannot call write after a stream was destroyed"), {
  ...
  emit(ev, ...args) {
    ...
    if (ev !== "error" && ev !== "close" && ev !== DESTROYED && this[DESTROYED]) return false;
      ...
      const ret =
        !this[SIGNAL] || this.listeners("error").length ? super.emit("error", data) : false;
      this[MAYBE_EMIT_END]();
      return ret;
    } else if (ev === "resume") {
...
var PathBase = class {
  ...
  #readdirMaybePromoteChild(e, c) {
    for (let p = c.provisional; p < c.length; p++) {
      const pchild = c[p];
      if ((this.nocase ? normalizeNocase(e.name) : normalize$1(e.name)) !== pchild.#matchName)
        continue;
...
var PathWin32 = class PathWin32 extends PathBase {
  ...
  getRoot(rootPath) {
    ...
    for (const [compare, root] of Object.entries(this.roots))
      if (this.sameRoot(rootPath, compare)) return (this.roots[rootPath] = root);
...
var Glob = class {
  ...
  constructor(pattern, opts) {
    ...
    const [matchSet, globParts] = this.pattern
      .map((p) => new Minimatch(p, mmo))
      .reduce(
        (set, m) => {
          set[0].push(...m.set);
...
function updateWorkspaceConfig(projectPath, workspaceInfo) {
  for (const pattern of workspaceInfo.workspacePatterns)
    if (minimatch(projectPath, pattern)) return;
  ...
  if (workspaceInfo.packageManager === PackageManager.pnpm)
    editYamlFile(path.join(workspaceInfo.rootDir, "pnpm-workspace.yaml"), (doc) => {
      ...
      if (!packages) packages = new YAMLSeq();
...
//#endregion
export {
  detectExistingEditor as a,
  detectEditorConflicts as i,
  updatePackageJsonWithDeps as n,
  selectEditor as o,
  updateWorkspaceConfig as r,
  writeEditorConfigs as s,
  detectWorkspace$1 as t,
};
