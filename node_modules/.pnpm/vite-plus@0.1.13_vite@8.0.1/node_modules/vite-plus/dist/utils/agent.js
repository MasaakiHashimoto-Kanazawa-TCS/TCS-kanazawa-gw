...
export async function selectAgentTargetPaths({ interactive, agent, onCancel }) {
...
export async function selectAgentTargetPath({ interactive, agent, onCancel }) {
  const targetPaths = await selectAgentTargetPaths({ interactive, agent, onCancel });
  return targetPaths?.[0];
}
export function detectExistingAgentTargetPaths(projectRoot) {
  const detectedPaths = [];
 ...
 */
export function updateExistingAgentInstructions(projectRoot) {
  const targetPaths = detectExistingAgentTargetPaths(projectRoot);
  if (!targetPaths) {
    return;
  }
...
export function resolveAgentTargetPath(agent) {
...
export function replaceMarkedAgentInstructionsSection(existing, incoming) {
