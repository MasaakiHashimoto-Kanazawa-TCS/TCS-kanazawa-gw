...
export function CompactAlertBanner({
  ...
  return (
    <div className={cn("space-y-2", className)}>
      {/* メインアラート */}
      <div className={cn("border rounded-lg p-4", severityStyles[primaryAlert.severity])}>
        <div className="flex items-start space-x-3">
          <div className="flex-shrink-0">{severityIcons[primaryAlert.severity]}</div>

          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between">
              <h4 className="text-sm font-medium">{primaryAlert.message}</h4>
              <div className="flex items-center space-x-2">
                ...
                <button
                  onClick={() =>
                    setExpandedAlert(expandedAlert === primaryAlert.id ? null : primaryAlert.id)
                  }
      ...
      {remainingCount > 0 && expandedAlert === primaryAlert.id && (
        <div className="space-y-1">
          {sortedAlerts.slice(1, 4).map((alert) => (
            <div
              key={alert.id}
              className={cn("border rounded p-2 text-sm", severityStyles[alert.severity])}
            >
              <div className="flex items-center justify-between">
                ...
                <div className="flex space-x-1">
                  ...
                  {onDismiss && (
                    <button onClick={() => onDismiss(alert.id)} className="text-xs hover:underline">
                      解除
                    </button>
                  )}
                </div>
