...
export function DashboardOverview({
  ...
  // アラート管理
  const { activeAlerts, unreadCount, acknowledgeAlert, dismissAlert, generateAlertsFromData } =
    useAlerts({
  ...
  const handleRefreshAll = async () => {
    await Promise.all([refreshPlants(), refreshTemperature(), refreshPH()]);
  };
  ...
  if (error && !selectedPlant) {
    ...
    return (
      <div className="py-12">
        <ErrorMessage error={error} retry={handleRefreshAll} variant="card" />
      </div>
    );
  }
    ...
    <div className={cn("space-y-6", className)}>
      ...
      {selectedPlant && (
        <div className="space-y-4">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">現在の環境データ</h2>
          <MetricsGrid
            metrics={{
      ...
      {selectedPlant && (
        <div className="space-y-4">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">植物の詳細情報</h2>
          <PlantCard
            plant={selectedPlant}
      ...
      {/* データタイプ選択（将来の拡張用） */}
      <div className="space-y-4">
        <h2 className="text-lg font-semibold text-gray-900 dark:text-white">データタイプ</h2>
      ...
      <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
        <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">システム状態</h3>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
          <div className="flex items-center space-x-2">
            <div className="w-2 h-2 bg-green-500 rounded-full"></div>
            <span className="text-gray-600 dark:text-gray-400">植物監視: 正常</span>
          </div>
          <div className="flex items-center space-x-2">
            <div
              className={cn(
